...
全部の「密度」＆「流速」の計算をgas.SPXによって計算したver.
"""
...
def func_MEPC_theta3_mode1(theta1, theta2, point1, point2, point4, lambda_12, eps=10e-6):
...
def func_MEPC_theta3_mode2(theta1, theta2,theta4, point1, point2, point4, lambda_12, eps=10e-6):
    lambda_o = np.tan((theta1+theta2)/2.)
    delta = 1.0
    # n=0
    while delta >= eps:
...
for i in range(1,int(num_ch_up)):### 20211022_sawada : 次の列の計算をしていないためにエラーが起きている
    ...
    while judge != judge_new or a == 0:
        ...
        array_rho[-1][i-1] = gas.density_mass
        array_V[-1][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
        ...
        while delta_c >= eps_c:
            ...
            array_p_3[-1][i-1] = array_p[-1][i-1]
            gas.SPX = s_post, array_p_3[-1][i-1], x_post
            array_rho_3[-1][i-1] = gas.density_mass
            array_V_3[-1][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
