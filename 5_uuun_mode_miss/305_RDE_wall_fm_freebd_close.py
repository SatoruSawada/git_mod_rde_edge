...
### C0 * x ** 4. + C1
deg1_up = angle_fm
deg2_up = (angle_sl+angle_fm)*3./10.
num_ch_up0 = int(num_ch_up*2./3.)
num_ch_up1 = num_ch_up - num_ch_up0
C1_up = deg1_up
C0_up = (deg2_up-C1_up)/((num_ch_up0-1)**4.)
array_sample0 = np.arange(num_ch_up0)
array_sample_up = array_sample0
# print("array_sample_up ============", array_sample_up * 360. / 2./ np.pi)
array_sample_up = array_sample_up * array_sample_up * array_sample_up * array_sample_up * C0_up + C1_up
C2 = array_sample_up[-1]
array_sample_up = np.delete(array_sample_up, -1)
array_theta_up = np.hstack((array_sample_up, np.linspace(C2, angle_sl, int(num_ch_up1+1))))
print("array_theta_up ============", array_theta_up * 360. / 2./ np.pi)
array_neu_up = array_theta_up - angle_fm
...
### C0 * x ** 4. + C1
deg1 = angle_fm
deg2 = (angle_bottom + angle_fm)*4./10.
num_ch_down0 = int(num_ch_down*2./3.)
num_ch_down1 = num_ch_down - num_ch_down0
C1 = deg1
C0 = (deg2-C1)/((num_ch_down0-1)**4.)
array_sample1 = np.arange(num_ch_down0)
array_sample_down = array_sample1
# print("array_sample_down ============", array_sample_down * 360. / 2./ np.pi)
array_sample_down = array_sample_down * array_sample_down * array_sample_down * array_sample_down * C0 + C1
C2 = array_sample_down[-1]
array_sample_down = np.delete(array_sample_down, -1)
array_theta_down = np.hstack((array_sample_down, np.linspace(C2, angle_bottom, num_ch_down1+1)))
print("array_theta_down ============", array_theta_down * 360. / 2./ np.pi)
