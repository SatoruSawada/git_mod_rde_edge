"""
特性線を追加する

1. 上からの特性線が底面に到達するまで計算
2. 特性線追加
3. 下からの特性線が上側境界条件に到達するまでの計算

特性線の状態量をしっかりと計算
追加の特性線の状態に関してarray_minus は同じ値にすべきなのか？


20211211_sawada_
これは最後の array_[j][i] でのエントロピーを計算する際はエントロピーの内挿が適応できない
教科書通りに Ao. Ro, To1, To2 等を用いて計算　-> エントロピーを算出（仮のエントロピー）
特性線 C+ & C- の計算では比熱比の変化を考慮したいから s を内挿して soundspeed_fr(gas) ???
どこまでが正しい判断なのか澤田にはわからない

20211212_sawada_
上記は間違い
ここではなんとかして再現しにかかる
エントロピー一定
エントロピー分布を考慮に入れるためにもやはり mass-stagnation method 必須
熱量的完全気体 gamma = gamma_post = const. (post-deto, post-obl-shock regions)
"""

## 意味ないけれども
## (__init__) -> 書いてもいいところ
## (__call__) -> 書いてはいけないところ


import numpy as np
import matplotlib.pylab as plt
from scipy import interpolate
np.set_printoptions(precision=2)
np.set_printoptions(suppress=True)

class CL_graph_setting:
    def __init__(self):
        #### ==================================================================================================================
        #### setting
        #### ==================================================================================================================
        # zoom = 0.6
        zoom = 4.
        # zoom = 1.
        #### x軸
        self.x_label = 'radial direction [-]'
        self.x_min = -0.02 * zoom        #### x軸最小値
        self.x_max = 0.14 * zoom        #### x軸最大値，目盛りの表示の都合でここに 0.00001 % 加算し申す
        self.x_main_dis = 0.02 * zoom   #### x軸主目盛り間隔
        # x_sub_num = 5       #### x軸主目盛り間の小目盛りの個数
        #### y軸
        self.y_label = 'azimuthal direction [-]'
        self.y_min = -0.01 * zoom        #### y軸最小値
        self.y_max = 0.07 * zoom        #### y軸最大値，目盛りの表示の都合でここに 0.00001 % 加算し申す
        self.y_main_dis = 0.01 * zoom   #### y軸主目盛り間隔
        # y_sub_num = 5       #### y軸主目盛り間の小目盛りの個数
        #### 軸の大きさ・太さ
        self.major_width = 1.5    #### 軸主目盛りの線幅
        self.major_size = 10      #### 軸主目盛りの長さ
        self.minor_width = 1.0    #### 軸補助目盛りの線幅
        self.minor_size = 5      #### 軸補助目盛りの長さ
        #### 書式
        self.letter_font = "Arial" #### 文字書式
        self.letter_size = 16      #### 文字サイズ
        self.number_font = "Arial" #### 数字書式
        self.number_size = 16      #### 数字サイズ
        #### marker
        self.marker_size = 1.
        self.marker_color = "black"
        #### 枠&図全体の設定
        self.fig_x_size = 16.     #### グラフ全体のx軸方向の大きさ
        self.fig_y_size = 8.     #### グラフ全体のy軸方向の大きさ
        self.axes_width = 1       #### 枠の太さ

        #### ================================================
        #### 原点を通過する軸の表示
        #### ================================================
        #### x軸方向
        self.fig_x_zero = True
        #### y 軸方向
        self.fig_y_zero = True

class CL_graph_create(CL_graph_setting):
    def __call__(self):
        #### ==================================================================================================================
        #### graph settings
        #### ==================================================================================================================
        ### font settings
        plt.rcParams["font.family"] = "Arial"      #全体のフォントを設定
        plt.rcParams["mathtext.fontset"] = "cm" 
        ### 目盛り外内側指定（外側(defolt) -> 'out'，内側 -> 'in'）
        plt.rcParams["xtick.direction"] = "in"               #x軸の目盛線を内向きへ
        plt.rcParams["ytick.direction"] = "in"               #y軸の目盛線を内向きへ
        ### 主目盛り
        plt.rcParams["xtick.major.width"] = self.major_width              #x軸主目盛り線の線幅
        plt.rcParams["ytick.major.width"] = self.major_width              #y軸主目盛り線の線幅
        plt.rcParams["xtick.major.size"] = self.major_size                #x軸主目盛り線の長さ
        plt.rcParams["ytick.major.size"] = self.major_size                #y軸主目盛り線の長さ
        ### 補助目盛り
        plt.rcParams["xtick.minor.visible"] = True           #x軸補助目盛りの追加
        plt.rcParams["ytick.minor.visible"] = True           #y軸補助目盛りの追加
        plt.rcParams["xtick.minor.width"] = self.minor_width              #x軸補助目盛り線の線幅
        plt.rcParams["ytick.minor.width"] = self.minor_width              #y軸補助目盛り線の線幅
        plt.rcParams["xtick.minor.size"] = self.minor_size                 #x軸補助目盛り線の長さ
        plt.rcParams["ytick.minor.size"] = self.minor_size                 #y軸補助目盛り線の長さ
        plt.rcParams["font.size"] = self.letter_size                       #フォントの大きさ
        plt.rcParams["axes.linewidth"] = self.axes_width                 #囲みの太さ
        ### 上，右側の目盛り表示
        plt.rcParams['xtick.top'] = True
        plt.rcParams['ytick.right'] = True

        #### ================================================
        #### set graph frame
        #### ================================================
        self.fig, self.ax = plt.subplots(1,1, figsize=(self.fig_x_size, self.fig_y_size)) #### 第1,2変数はfig,axの配列の数を指定
        #### x_axis
        x_range_max_array = np.arange(0.0, self.x_max*1.00000000001, self.x_main_dis)
        x_range_min_array = np.arange(0.0, abs(self.x_min)*1.00000000001, self.x_main_dis) * (-1)
        x_range_array = np.sort(np.unique(np.hstack([x_range_max_array, x_range_min_array])))
        self.ax.set_xticks(x_range_array)
        self.ax.set_xlim(self.x_min, self.x_max*1.00000000001)
        self.ax.set_xlabel(self.x_label)
        #### y_axis
        y_range_max_array = np.arange(0.0, self.y_max*1.00000000001, self.y_main_dis)
        y_range_min_array = np.arange(0.0, abs(self.y_min)*1.00000000001, self.y_main_dis) * (-1)
        y_range_array = np.sort(np.unique(np.hstack([y_range_max_array, y_range_min_array])))
        self.ax.set_yticks(y_range_array)
        self.ax.set_ylim(self.y_min, self.y_max*1.00000000001)
        self.ax.set_ylabel(self.y_label)

    def func_rde_inlet(self):
        list_x_axis_x = [-10e1, 10e1]
        list_x_axis_y = [0., 0.]
        self.ax.plot(list_x_axis_x, list_x_axis_y, color='k')

    def func_rde_exit(self, rde_l):
        list_x_axis_x = [-10e1, 10e1]
        list_x_axis_y = [rde_l, rde_l]
        self.ax.plot(list_x_axis_x, list_x_axis_y, color='k')

    def func_plot_add(self, list_x, list_y, color=None):
        #### ==================================================================================================================
        #### graph depict
        #### ==================================================================================================================
        # colorlist = ["r", "g", "b", "c", "m", "y", "k", "w"]
        # self.ax.scatter(list_x, list_y, s=marker_size, c=marker_color)
        if color==None:
            self.ax.plot(list_x, list_y)
        else:
            self.ax.plot(list_x, list_y, c=color)

    def func_scatter_add(self,array_x,array_y, color='k'):
        array_x = array_x.flatten()
        array_y = array_y.flatten()
        self.ax.scatter(array_x,array_y,color=color)

    def func_show(self):
        plt.show()


#### ================================================
#### 関数
#### ================================================
def func_kainokoushiki(a, b, c):
    return (-b+np.sqrt(b**2.-4.*a*c))/2.*a

def func_cross(list_a, list_b):
    a0 = list_a[0]
    a1 = list_a[1]
    b0 = list_b[0]
    b1 = list_b[1]
    cross_x = -(b0-b1)/(a0-a1)
    cross_y = a0 * cross_x + b0
    return cross_x, cross_y

def func_intercept(slope, point):
    x_A = point[0]
    y_A = point[1]
    intercept = y_A - slope * x_A
    return intercept 

def func_cross_gas_dynamics(point1, point2, lambda1, lambda2): ### ごめん，くそみたいな名前で
    x = ((point1[1]-point2[1])-(lambda1*point1[0]-lambda2*point2[0]))/(lambda2-lambda1)
    y = point1[1] - lambda1 * point1[0] + lambda1 * x
    return x, y

def func_MEPC_theta3_mode1(theta1, theta2, point1, point2, point4, lambda_12, lambda_o, eps=10e-6):
    lambda_o = np.tan((theta1+theta2)/2.)
    delta = 1.0
    while delta >= eps:
        x, y = func_cross_gas_dynamics(point2, point4, lambda_12,lambda_o)
        theta = theta2 + (y-point2[1])/(point1[1]-point2[1]) * (theta1-theta2)
        lambda_o_new = np.tan(theta)
        delta = abs((lambda_o-lambda_o_new)/lambda_o)
        lambda_o = lambda_o_new
    return x, y, theta, lambda_o

def func_MEPC_theta3_mode2(theta1, theta2,theta4, point1, point2, point4, lambda_12, lambda_o, eps=10e-6):
    delta = 1.0
    while delta >= eps:
        x, y = func_cross_gas_dynamics(point2, point4, lambda_12,lambda_o)
        theta = theta2 + (y-point2[1])/(point1[1]-point2[1]) * (theta1-theta2)
        lambda_o_new = np.tan((theta+theta4)/2.)
        delta = abs((lambda_o-lambda_o_new)/lambda_o)
        lambda_o = lambda_o_new
    return x, y, theta, lambda_o


#------------------------------------------------------------------
#### 0. parameters
#------------------------------------------------------------------
rde_l = 0.06 # [-]: RDE's combustion chamber length normalized by injection fill height 
angle_fm = 10. / 360. * 2. * np.pi # deto_angle - np.pi/2.
angle_dw = angle_fm + 90. / 360. * 2. * np.pi # [rad]: detonation angle from horizontal axis (theta axis)
angle_sl = 43. / 360. * 2. * np.pi ### [rad]: slip line angle from horizontal axis (theta axis)
angle_sh = 60. / 360. * 2. * np.pi ### [rad]: oblique shock angle from horizontal axis (theta axis)
angle_bottom = 0. * 2. * np.pi /360.

#------------------------------------------------------------------
#### 0. graph_prepare
#------------------------------------------------------------------
## x deto波を「1」として無次元化
## y deto波を「1」として無次元化
graph0 = CL_graph_create()
graph0() # reset
graph0.func_rde_inlet()
graph0.func_rde_exit(rde_l)

#------------------------------------------------------------------
#### 0. assumptions for detonation wave
#------------------------------------------------------------------
## deto波と燃焼室底面の接点はどの条件であろうと不変である -> 原点 （本番でもこのつもり）
## この計算では「deto_height」固定
height_dw = 0.02 # [-]: injection fill height normalized by deto_height (z axis)
array_point_dw = (height_dw*np.tan(-(angle_dw-np.pi/2.)), height_dw)
graph0.func_plot_add((0., array_point_dw[0]), (0., array_point_dw[1]), color="r")

#------------------------------------------------------------------
#### 0. assumptions for fresh mixture layer
#------------------------------------------------------------------
## 「detonation wave」と「fresh mixture layer」のどちらを先に描画するべきなのか分からないからとりあえず
slope_fm = np.tan(angle_fm)
intercept_fm = func_intercept(slope_fm, array_point_dw)
x_cross, y_cross = func_cross((0., slope_fm), (0., intercept_fm))
graph0.func_plot_add((array_point_dw[0], x_cross), (array_point_dw[1], y_cross), color="b")

#------------------------------------------------------------------
#### 0. assumptions for slip line
#------------------------------------------------------------------
slope_sl = np.tan(angle_sl)
intercept_sl = func_intercept(slope_sl, array_point_dw)
x_cross, y_cross = func_cross((0., slope_sl), (rde_l, intercept_sl))
graph0.func_plot_add((array_point_dw[0], x_cross), (array_point_dw[1], y_cross), color="b")

#------------------------------------------------------------------
#### 仮定：
#------------------------------------------------------------------
import cantera as ct
from sdtoolbox.thermo import soundspeed_fr, soundspeed_eq
mech = 'gri30_highT.cti'
gas = ct.Solution(mech)
ER = 1.
coef_C2H4 = 1
coef_O2 = 3
mol_ratio = coef_C2H4/coef_O2*ER
fuel = 'C2H4'
oxygen = 'O2'
q = fuel + ':' + str(mol_ratio) + ', ' + oxygen + ':1.0';

######## State 0 - plenum
P_ple = 202871.4251657753
T_ple = 300.10181107013716 #(K)
rho_ple = 2.5189165307697503 #(kg/m3)
a_fr_ple = 328.2601530165985 #(m/s)
h_ple = 425176.32528145495 #(J/kg)
s_ple = 6700.9149607432355 #(J/kg K)
gamma_fr_ple = 1.339709604023228
R_ple = 268.10468909303626

######## Layer detonation computation for gri30_highT.cti with composition C2H4:0.3333333333333333, O2:1.0
######## State 1 - Initial state of reacting layer
P_pre = 109278.90298573895 #(Pa)
T_pre = 256.5331450748727 #(K)
rho_pre = 1.5872829592437772 #(kg/m3)
a_fr_pre = 305.2743367541806 #(m/s)
h_pre = 379882.37074241653 #(J/kg)
s_pre = 6703.726763985172 #(J/kg K)
gamma_fr_pre = 1.3554605754094926
R_pre = 268.10468909303626

######## State 2 - CJ 
CJ_speed = 2386.591152672143 #(m/s)
P_post = 4270213.871314236 #(Pa)
T_post = 3979.0578618774894 #(K)
rho_post = 2.9407834010709206 #(kg/m3)
h_post = 2398111.5517674736 #(J/kg K)
s_post = 11619.80436302621 #(J/kg K)
w_post = 1288.1586130889275 #(m/s)
u_post = 1098.4325395832154 #(m/s)
a_eq_post = 1286.5955416302932 #(m/s)
# M_post = w_post/a_eq_post
gamma_eq_post = 1.1399806928419203
x_post = [5.66228974e-02, 5.63248708e-02, 7.06901613e-02, 1.03138136e-01,\
 1.26319510e-01, 2.19178349e-01, 3.61837693e-04, 2.43583610e-05,\
 9.87591241e-09, 2.04220827e-09, 1.05416894e-09, 1.41773264e-10,\
 4.55493375e-10, 3.81726776e-11, 2.55874711e-01, 1.11443091e-01,\
 2.18018804e-05, 2.62082032e-07, 2.03546674e-10, 1.69701442e-11,\
 4.93273199e-12, 7.88957542e-13, 1.64881692e-12, 1.42225878e-15,\
 1.09671898e-16, 1.42327644e-19, 2.28514968e-21, 2.89655410e-11,\
 4.99422907e-12, 1.24757748e-13, 0.00000000e+00, 0.00000000e+00,\
 0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00,\
 0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00,\
 0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00,\
 0.00000000e+00, 0.00000000e+00, 0.00000000e+00, 0.00000000e+00,\
 0.00000000e+00, 2.36527675e-29, 3.48786919e-31, 2.78780489e-15,\
 1.59215284e-16]

####### total enthalpy
U_post = w_post
h_post_U_post = (h_post + U_post**2./2.) # SDT

def func_delta_M(neu_target, M, gamma):
    part1 = ((gamma+1.)/(gamma-1.))**(1./2.)
    part2 = np.arctan(((gamma-1.)/(gamma+1.)*(M**2.-1.))**(1./2.))
    part3 = np.arctan((M**2.-1.)**(1./2.))
    neu_cal = part1*part2-part3
    delta_neu = neu_target - neu_cal
    return delta_neu

def func_M2neu(M, gamma):
    part1 = ((gamma+1.)/(gamma-1.))**(1./2.)
    part2 = np.arctan(((gamma-1.)/(gamma+1.)*(M**2.-1.))**(1./2.))
    part3 = np.arctan((M**2.-1.)**(1./2.))
    neu = part1*part2-part3
    return neu

def func_neu2M(neu_target, gamma, eps = 10e-10):
    M_a = 1.2
    M_b = 1.4
    dM_a = func_delta_M(neu_target,M_a, gamma)
    dM_b = func_delta_M(neu_target,M_b, gamma)
    while abs(dM_b) > eps:
        M_s = (M_a * dM_b - M_b * dM_a)/(dM_b - dM_a)
        M_a, M_b = M_b, M_s
        dM_a = dM_b
        dM_b = func_delta_M(neu_target, M_b, gamma)
    M_result = M_b
    return M_result

def func_dM(M,p):
    gas.SPX = s_post, p, x_post
    v2 = 2.0*(h_post_U_post - gas.enthalpy_mass)
    v = np.sqrt(v2)
    dM = abs(M - v/soundspeed_fr(gas)) / M
    return dM

def func_M2P(M, eps=10e-10):
    P_a = 0.001 * 101300
    P_b = 0.002 * 101300
    dM_b = func_dM(M, P_b)
    dM_a = func_dM(M, P_a)
    while abs(dM_b) > eps:
        P_s = (P_a * dM_b - P_b * dM_a)/(dM_b - dM_a)
        P_a, P_b = P_b, P_s
        dM_a = dM_b
        dM_b = func_dM(M, P_b)
    gas.SPX = s_post, P_b,x_post
    T = gas.T
    R = ct.gas_constant / gas.mean_molecular_weight
    rho = gas.density
    a_fr = soundspeed_fr(gas)
    V = M * a_fr
    gamma = a_fr**2.*rho/P_b
    return P_b, T, R, rho, a_fr, V, gamma 


### ================================================================================================
### ================================================================================================
### 既燃ガス層（滑り面ー滑り面）の行列の設定
### ================================================================================================
### ================================================================================================

#------------------------------------------------------------------
#### 1. characteristic lines -1st
#------------------------------------------------------------------
### num_ch_up & num_ch_down が小さすぎても問題（num_ch_up & num_ch_down >= 7）
### i方向（横）にtheta-neu=const.確認
### j方向（縦）にtheta+neu=const.確認
num_ch_up = 20 # number of initial characteristic lines (upper side)
num_ch_down = 10 # number of initial characteristic lines (down side)
num_ch_add = 5
# init_theta_delta = 10e-11
inflow_distance = 0.
array_x_fm = np.empty(0)
array_y_fm = np.empty(0)
array_theta_fm = np.empty(0)
array_x_sl = np.empty(0)
array_y_sl = np.empty(0)
array_zero0 = np.zeros((int(num_ch_down),int(num_ch_up-1)))
array_zero1 = np.zeros((int(num_ch_up + num_ch_down - 1),int(num_ch_down + num_ch_add)))

### x for characteristics
array_x_up = np.ones((int(num_ch_up))) * array_point_dw[0]
array_x = np.flipud(np.diag(array_x_up))
array_x = np.delete(array_x,-1,0)
array_x_down = np.zeros((int(num_ch_down)))
array_x_down = np.transpose(np.array([array_x_down]))
array_x_down = np.hstack((array_x_down,array_zero0))
array_x = np.vstack((array_x,array_x_down))
array_x = np.hstack((array_x, array_zero1))
del array_x_up
del array_x_down

### y for characteristics
array_y_up = np.ones((int(num_ch_up))) * array_point_dw[1]
array_y = np.flipud(np.diag(array_y_up))
array_y = np.delete(array_y,-1,0)
array_y_down = np.zeros((int(num_ch_down)))
array_y_down = np.transpose(np.array([array_y_down]))
array_y_down = np.hstack((array_y_down,array_zero0))
array_y = np.vstack((array_y,array_y_down))
array_y = np.hstack((array_y, array_zero1))
del array_y_up
del array_y_down

### ============================================================================================ 20211018_sawada
### ============================================================================================ 初期値の設定がよくわからない
### ============================================================================================ とりあえずリーマン不変量で
### マッハ数：等差
M_post = 1.
neu_sl = angle_sl - (angle_fm - func_M2neu(M_post,gamma_eq_post))
### 小賢しいが gamma 繰り返し計算で補正，ごめん
gamma_sl = 1.2
delta_gamma = 1.0
eps_gamma = 10e-10
while delta_gamma >= eps_gamma:
    M_sl = func_neu2M(neu_sl,gamma_sl)
    P_sl, T, R, rho, a_fr, V, gamma_sl_new = func_M2P(M_sl)
    delta_gamma = abs(gamma_sl-gamma_sl_new)/gamma_sl
    gamma_sl = gamma_sl_new
array_M_up = np.linspace(M_post,M_sl,num_ch_up)

array_neu_up = np.zeros((int(num_ch_up)))
array_alpha_up = np.zeros((int(num_ch_up)))
array_p_up = np.zeros((int(num_ch_up)))
array_t_up = np.zeros((int(num_ch_up)))
array_R_up = np.zeros((int(num_ch_up)))
array_rho_up = np.zeros((int(num_ch_up)))
array_a_fr_up = np.zeros((int(num_ch_up)))
array_V_up = np.zeros((int(num_ch_up)))
array_gamma_up = np.zeros((int(num_ch_up)))
for i0 in range(int(num_ch_up)):
    array_p_up[i0], \
        array_t_up[i0], \
            array_R_up[i0], \
                array_rho_up[i0], \
                    array_a_fr_up[i0], \
                        array_V_up[i0], \
                            array_gamma_up[i0] = func_M2P(array_M_up[i0])
    array_neu_up[i0] = func_M2neu(array_M_up[i0], array_gamma_up[i0])
    array_alpha_up[i0] = np.arcsin(1./array_M_up[i0])
array_theta_up = array_neu_up + (angle_fm - func_M2neu(M_post,gamma_eq_post))

array_theta = np.flipud(np.diag(array_theta_up))
array_neu = np.flipud(np.diag(array_neu_up))
array_M = np.flipud(np.diag(array_M_up))
array_alpha = np.flipud(np.diag(array_alpha_up))
array_p = np.flipud(np.diag(array_p_up))
array_t = np.flipud(np.diag(array_t_up))
array_R = np.flipud(np.diag(array_R_up))
array_rho = np.flipud(np.diag(array_rho_up))
array_a_fr = np.flipud(np.diag(array_a_fr_up))
array_V = np.flipud(np.diag(array_V_up))
array_gamma = np.flipud(np.diag(array_gamma_up))

#============================================================================
array_theta = np.delete(array_theta,-1,0)
array_neu = np.delete(array_neu,-1,0)
array_M = np.delete(array_M,-1,0)
array_alpha = np.delete(array_alpha,-1,0)
array_p = np.delete(array_p,-1,0)
array_t = np.delete(array_t,-1,0)
array_R = np.delete(array_R,-1,0)
array_rho = np.delete(array_rho,-1,0)
array_a_fr = np.delete(array_a_fr,-1,0)
array_V = np.delete(array_V,-1,0)
array_gamma = np.delete(array_gamma,-1,0)

#============================================================================
### マッハ数：等差
M_post = 1.
neu_bottom = -angle_bottom + (angle_fm + func_M2neu(M_post,gamma_eq_post))
### 小賢しいが gamma 繰り返し計算で補正，ごめん

gamma_bottom = 1.2
delta_gamma = 1.0
while delta_gamma >= eps_gamma:
    M_bottom = func_neu2M(neu_bottom,gamma_bottom)
    P_bottom, T, R, rho, a_fr, V, gamma_bottom_new = func_M2P(M_bottom)
    delta_gamma = abs(gamma_bottom-gamma_bottom_new)/gamma_bottom
    gamma_bottom = gamma_bottom_new
array_M_down = np.linspace(M_post,M_bottom,num_ch_down)

array_neu_down = np.zeros((int(num_ch_down)))
array_alpha_down = np.zeros((int(num_ch_down)))
array_p_down = np.zeros((int(num_ch_down)))
array_t_down = np.zeros((int(num_ch_down)))
array_R_down = np.zeros((int(num_ch_down)))
array_rho_down = np.zeros((int(num_ch_down)))
array_a_fr_down = np.zeros((int(num_ch_down)))
array_V_down = np.zeros((int(num_ch_down)))
array_gamma_down = np.zeros((int(num_ch_down)))
for i0 in range(int(num_ch_down)):
    array_p_down[i0], \
        array_t_down[i0], \
            array_R_down[i0], \
                array_rho_down[i0], \
                    array_a_fr_down[i0], \
                        array_V_down[i0], \
                            array_gamma_down[i0] = func_M2P(array_M_down[i0])
    array_neu_down[i0] = func_M2neu(array_M_down[i0], array_gamma_down[i0])
    array_alpha_down[i0] = np.arcsin(1./array_M_down[i0])
array_theta_down = -array_neu_down + (angle_fm + func_M2neu(M_post,gamma_eq_post))

#============================================================================
### =====
array_theta_down = np.transpose(np.array([array_theta_down]))
array_theta_down = np.hstack((array_theta_down,array_zero0))
array_theta = np.vstack((array_theta,array_theta_down))
array_theta = np.hstack((array_theta, array_zero1))
### =====
array_M_down = np.transpose(np.array([array_M_down]))
array_M_down = np.hstack((array_M_down,array_zero0))
array_M = np.vstack((array_M,array_M_down))
array_M = np.hstack((array_M, array_zero1))
### =====
array_alpha_down = np.transpose(np.array([array_alpha_down]))
array_alpha_down = np.hstack((array_alpha_down,array_zero0))
array_alpha = np.vstack((array_alpha,array_alpha_down))
array_alpha = np.hstack((array_alpha, array_zero1))
### =====
array_p_down = np.transpose(np.array([array_p_down]))
array_p_down = np.hstack((array_p_down,array_zero0))
array_p = np.vstack((array_p,array_p_down))
array_p = np.hstack((array_p, array_zero1))
### =====
array_t_down = np.transpose(np.array([array_t_down]))
array_t_down = np.hstack((array_t_down,array_zero0))
array_t = np.vstack((array_t,array_t_down))
array_t = np.hstack((array_t, array_zero1))
### =====
array_R_down = np.transpose(np.array([array_R_down]))
array_R_down = np.hstack((array_R_down,array_zero0))
array_R = np.vstack((array_R,array_R_down))
array_R = np.hstack((array_R, array_zero1))
### =====
array_rho_down = np.transpose(np.array([array_rho_down]))
array_rho_down = np.hstack((array_rho_down,array_zero0))
array_rho = np.vstack((array_rho,array_rho_down))
array_rho = np.hstack((array_rho, array_zero1))
### =====
array_a_fr_down = np.transpose(np.array([array_a_fr_down]))
array_a_fr_down = np.hstack((array_a_fr_down,array_zero0))
array_a_fr = np.vstack((array_a_fr,array_a_fr_down))
array_a_fr = np.hstack((array_a_fr, array_zero1))
### =====
array_V_down = np.transpose(np.array([array_V_down]))
array_V_down = np.hstack((array_V_down,array_zero0))
array_V = np.vstack((array_V,array_V_down))
array_V = np.hstack((array_V, array_zero1))
### =====
array_gamma_down = np.transpose(np.array([array_gamma_down]))
array_gamma_down = np.hstack((array_gamma_down,array_zero0))
array_gamma = np.vstack((array_gamma,array_gamma_down))
array_gamma = np.hstack((array_gamma, array_zero1))

### ============================================================================
del array_theta_up; del array_theta_down; del array_neu_up; del array_neu_down
del array_M_up; del array_M_down; del array_alpha_up; del array_alpha_down
del array_p_up; del array_p_down; del array_t_up; del array_t_down
del array_R_up; del array_R_down; del array_rho_up; del array_rho_down
del array_a_fr_up; del array_a_fr_down; del array_V_up; del array_V_down
del array_gamma_up; del array_gamma_down
### ====

### C+ 上のパラメーター？？？
array_T_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_p_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_theta_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_V_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_rho_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_y_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_a_fr_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_M_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_alpha_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_lambda_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_Q_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_S_plus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))

### C- 上のパラメーター？？？
array_T_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_p_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_theta_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_V_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_rho_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_y_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_a_fr_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_M_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_alpha_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_lambda_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_Q_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_S_minus = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))

### point3
array_x_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_y_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_theta_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_p_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_rho_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_V_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_a_fr_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_gamma_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))

### point_o
array_lambda_12 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_lambda_o = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_p_o = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_rho_o = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_V_o = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_a_fr_3 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_R_o = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_A_o = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_T_o1 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))
array_T_o2 = np.zeros((int(num_ch_up+num_ch_down-1), int(num_ch_up + num_ch_down + num_ch_add)))

### ================================================================================================
### ================================================================================================
### 未燃ガス層の行列の設定
### ================================================================================================
### ================================================================================================
num_ch_fm_tate = 5 ### including (top_point) & (bottom_point)
num_ch_fm_yoko = 30

array_x_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_y_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_s_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_h_fm_lab_fixed = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_h_fm_shock_fixed = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))

array_theta_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_neu_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_M_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_alpha_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_p_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_t_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_R_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_rho_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_a_fr_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_V_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_gamma_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))

### C+ 上のパラメーター？？？
array_T_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_p_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_theta_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_V_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_rho_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_y_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_a_fr_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_M_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_alpha_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_lambda_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_Q_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_S_plus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))

### C- 上のパラメーター？？？
array_T_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_p_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_theta_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_V_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_rho_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_y_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_a_fr_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_M_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_alpha_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_lambda_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_Q_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_S_minus_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))

### point3
array_x_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_y_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_theta_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_p_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_rho_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_V_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_a_fr_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_gamma_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))

### point_o
array_lambda_12_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_lambda_o_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_p_o_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_rho_o_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_V_o_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_a_fr_3_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_R_o_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_A_o_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_T_o1_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))
array_T_o2_fm = np.zeros((int(num_ch_fm_tate), int(num_ch_fm_yoko)))

### ================================================================================================
### ================================================================================================
### 既燃ガス層（斜め衝撃波ー滑り面）の行列の設定
### ================================================================================================
### ================================================================================================

i_fm = 0

mode_top = 0
mode_bottom = 0
j_bottom = num_ch_up+num_ch_down-2
eps_c = 10e-10

for i in range(1,int(num_ch_up)):### 20211022_sawada : 次の列の計算をしていないためにエラーが起きている
    for j in range(int(num_ch_up-i), int(j_bottom+1)):

        if j < j_bottom:
            
            ### a_interior
            ### =====================================================================
            ### predictor
            ### =====================================================================
            #####################################################################################################(c)
            ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
            array_lambda_plus[j+1][i-1] = np.tan(array_theta[j+1][i-1]+array_alpha[j+1][i-1])
            array_lambda_minus[j-1][i] = np.tan(array_theta[j-1][i]-array_alpha[j-1][i])
            ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
            array_Q_plus[j+1][i-1] = np.sqrt((array_M[j+1][i-1])**2.-1.) / (array_rho[j+1][i-1]*array_V[j+1][i-1]**2.)
            array_Q_minus[j-1][i] = np.sqrt((array_M[j-1][i])**2.-1.) / (array_rho[j-1][i]*array_V[j-1][i]**2.)
            #####################################################################################################(d)
            ### eq17dot44_eq17dot45
            array_x[j][i], array_y[j][i] = func_cross_gas_dynamics(\
                (array_x[j-1][i], array_y[j-1][i]),\
                    (array_x[j+1][i-1], array_y[j+1][i-1]),\
                        array_lambda_minus[j-1][i], \
                            array_lambda_plus[j+1][i-1])
            ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
            array_T_plus[j+1][i-1] = -array_S_plus[j+1][i-1] * (array_x[j][i]-array_x[j+1][i-1]) + \
                array_Q_plus[j+1][i-1] * array_p[j+1][i-1] + array_theta[j+1][i-1]
            array_T_minus[j-1][i] = -array_S_minus[j-1][i] * (array_x[j][i]-array_x[j-1][i]) + \
                array_Q_minus[j-1][i] * array_p[j-1][i] - array_theta[j-1][i]
            #####################################################################################################(e)
            ### eq17dot49 (the modified euler predictor-corrector)
            ### eq17dot43_eq17dot46
            array_lambda_12[j-1][i] = (array_y[j+1][i-1] - array_y[j-1][i]) / (array_x[j+1][i-1] - array_x[j-1][i])
            array_lambda_o[j][i-1] = np.tan((array_theta[j-1][i] + array_theta[j+1][i-1])/2.)
            array_x_3[j][i-1],\
                array_y_3[j][i-1],\
                    array_theta_3[j][i-1],\
                        array_lambda_o[j][i-1] = func_MEPC_theta3_mode1(\
                array_theta[j-1][i],\
                    array_theta[j+1][i-1],\
                        (array_x[j-1][i], array_y[j-1][i]),\
                            (array_x[j+1][i-1], array_y[j+1][i-1]),\
                                (array_x[j][i], array_y[j][i]),\
                                    array_lambda_12[j-1][i],\
                                        array_lambda_o[j][i-1])
            ### interpolating for the remaining flow properties gives... (p.203) 
            array_p_3[j][i-1] = array_p[j+1][i-1]+(array_y_3[j][i-1]-array_y[j+1][i-1])/\
                (array_y[j-1][i]-array_y[j+1][i-1])*(array_p[j-1][i]-array_p[j+1][i-1])
            gas.SPX = s_post, array_p_3[j][i-1], x_post
            array_rho_3[j][i-1] = gas.density_mass
            array_V_3[j][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
            #####################################################################################################(f)
            array_a_fr_3[j][i-1] = soundspeed_fr(gas)
            array_R_o[j][i-1] = array_rho_3[j][i-1] * array_V_3[j][i-1]
            array_A_o[j][i-1] = array_a_fr_3[j][i-1] ** 2.
            array_T_o1[j][i-1] = array_R_o[j][i-1] * array_V_3[j][i-1] + array_p_3[j][i-1] # using state3
            array_T_o2[j][i-1] = array_p_3[j][i-1] - array_A_o[j][i-1] * array_rho_3[j][i-1] # using state3
            ### eq.(g) & (h) for calculating p4, theta4
            array_p[j][i] = (array_T_plus[j+1][i-1] + array_T_minus[j-1][i]) / (array_Q_plus[j+1][i-1] + array_Q_minus[j-1][i])
            array_theta[j][i] = array_T_plus[j+1][i-1] - array_Q_plus[j+1][i-1] * array_p[j][i]
            ## eq.(i) & (j) for calculating V4, rho4
            # gas.SPX = s_post,array_p[j][i],x_post
            # array_V[j][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
            # array_rho[j][i] = gas.density_mass
            ### eq.(i) & (j) for calculating V4, rho4
            array_V[j][i] = (array_T_o1[j][i-1]-array_p[j][i]) / array_R_o[j][i-1]
            array_rho[j][i] = (array_p[j][i]-array_T_o2[j][i-1]) / array_A_o[j][i-1]
            gas.DPX = array_rho[j][i], array_p[j][i], x_post
            array_s[j][i] = gas.entropy_mass ##### お試し，やはり基礎方程式を満たすことを最優先に考えるべきでは？
            ### set predictor
            theta_3 = array_theta_3[j][i-1]
            delta_c = 1.0
            ### =====================================================================
            ### corrector : 全て入れなおせているのだろうか？
            ### =====================================================================
            while delta_c >= eps_c:
                # print('i =', i, '/// j =', j)
                #####################################################################################################(g)
                ### along Mach line 24 (C+)
                array_p_plus[j+1][i-1] = (array_p[j+1][i-1] + array_p[j][i]) /2.
                array_theta_plus[j+1][i-1] = (array_theta[j+1][i-1] + array_theta[j][i]) /2.
                gas.SPX = s_post, array_p_plus[j+1][i-1], x_post
                array_V_plus[j+1][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                array_rho_plus[j+1][i-1] = gas.density_mass
                array_y_plus[j+1][i-1] = (array_y[j+1][i-1] + array_y[j][i]) /2.
                array_a_fr_plus[j+1][i-1] = soundspeed_fr(gas)
                array_M_plus[j+1][i-1] = array_V_plus[j+1][i-1] / array_a_fr_plus[j+1][i-1]
                array_alpha_plus[j+1][i-1] = np.arcsin(1./array_M_plus[j+1][i-1])
                array_lambda_plus[j+1][i-1] = np.tan(array_theta_plus[j+1][i-1]+array_alpha_plus[j+1][i-1])
                array_Q_plus[j+1][i-1] = np.sqrt(array_M_plus[j+1][i-1]**2.-1.) / (array_rho_plus[j+1][i-1]*array_V_plus[j+1][i-1]**2.)
                ### along Mach line 14 (C-)
                array_p_minus[j-1][i] = (array_p[j-1][i] + array_p[j][i]) /2.
                array_theta_minus[j-1][i] = (array_theta[j-1][i] + array_theta[j][i]) /2.
                gas.SPX = s_post, array_p_minus[j-1][i], x_post
                array_V_minus[j-1][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                array_rho_minus[j-1][i] = gas.density_mass
                array_y_minus[j-1][i] = (array_y[j-1][i] + array_y[j][i]) /2.
                array_a_fr_minus[j-1][i] = soundspeed_fr(gas)
                array_M_minus[j-1][i] = array_V_minus[j-1][i] / array_a_fr_minus[j-1][i]
                array_alpha_minus[j-1][i] = np.arcsin(1./array_M_minus[j-1][i])
                array_lambda_minus[j-1][i] = np.tan(array_theta_minus[j-1][i]-array_alpha_minus[j-1][i])
                array_Q_minus[j-1][i] = np.sqrt(array_M_minus[j-1][i]**2.-1.) / (array_rho_minus[j-1][i]*array_V_minus[j-1][i]**2.)
                #####################################################################################################(h)
                ### eq17dot44_eq17dot45
                array_x[j][i], array_y[j][i] = func_cross_gas_dynamics(\
                    (array_x[j-1][i], array_y[j-1][i]),\
                        (array_x[j+1][i-1], array_y[j+1][i-1]),\
                            array_lambda_minus[j-1][i], \
                                array_lambda_plus[j+1][i-1])
                ### T+ & T- - eq17dot52_eq17dot53 (first step predictor
                array_T_plus[j+1][i-1] = -array_S_plus[j+1][i-1] * (array_x[j][i]-array_x[j+1][i-1]) + \
                    array_Q_plus[j+1][i-1] * array_p[j+1][i-1] + array_theta[j+1][i-1]
                array_T_minus[j-1][i] = -array_S_minus[j-1][i] * (array_x[j][i]-array_x[j-1][i]) + \
                    array_Q_minus[j-1][i] * array_p[j-1][i] - array_theta[j-1][i]
                #####################################################################################################(i)
                ### eq17dot49 (the modified euler predictor-corrector)
                ### eq17dot43_eq17dot46
                ### (theta3, theta4) -> corrector
                array_lambda_o[j][i-1] = np.tan((array_theta_3[j][i-1]+array_theta[j][i])/2.)
                array_x_3[j][i-1],\
                    array_y_3[j][i-1],\
                        array_theta_3[j][i-1],\
                            array_lambda_o[j][i-1] = func_MEPC_theta3_mode2(\
                    array_theta[j-1][i],\
                        array_theta[j+1][i-1],\
                        array_theta[j][i],\
                            (array_x[j-1][i], array_y[j-1][i]),\
                                (array_x[j+1][i-1], array_y[j+1][i-1]),\
                                    (array_x[j][i], array_y[j][i]),\
                                        array_lambda_12[j-1][i],\
                                            array_lambda_o[j][i-1])
                ### interpolating for the remaining flow properties gives... (p.203)
                array_p_3[j][i-1] = array_p[j+1][i-1]+(array_y_3[j][i-1]-array_y[j+1][i-1])/(array_y[j-1][i]-array_y[j+1][i-1])*(array_p[j-1][i]-array_p[j+1][i-1])
                gas.SPX = s_post, array_p_3[j][i-1], x_post
                array_rho_3[j][i-1] = gas.density_mass
                array_V_3[j][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                #####################################################################################################(j)
                ### (start) ここで計算が「predictor」と「corrector」で異なる
                array_p_o[j][i-1] = (array_p_3[j][i-1] + array_p[j][i]) / 2.
                gas.SPX = s_post, array_p_o[j][i-1], x_post
                array_rho_o[j][i-1] = gas.density_mass
                array_V_o[j][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                ### (end)
                array_a_fr_3[j][i-1] = soundspeed_fr(gas)
                array_R_o[j][i-1] = array_rho_3[j][i-1] * array_V_3[j][i-1]
                array_A_o[j][i-1] = array_a_fr_3[j][i-1] ** 2.
                array_T_o1[j][i-1] = array_R_o[j][i-1] * array_V_3[j][i-1] + array_p_3[j][i-1] # using state3
                array_T_o2[j][i-1] = array_p_3[j][i-1] - array_A_o[j][i-1] * array_rho_3[j][i-1] # using state3
                ### eq.(g) & (h) for calculating p4, theta4
                array_p[j][i] = (array_T_plus[j+1][i-1] + array_T_minus[j-1][i]) / (array_Q_plus[j+1][i-1] + array_Q_minus[j-1][i])
                array_theta[j][i] = array_T_plus[j+1][i-1] - array_Q_plus[j+1][i-1] * array_p[j][i]
                # ### eq.(i) & (j) for calculating V4, rho4
                # gas.SPX = s_post, array_p[j][i], x_post
                # array_V[j][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                # array_rho[j][i] = gas.density_mass
                ### eq.(i) & (j) for calculating V4, rho4
                array_V[j][i] = (array_T_o1[j][i-1]-array_p[j][i]) / array_R_o[j][i-1]
                array_rho[j][i] = (array_p[j][i]-array_T_o2[j][i-1]) / array_A_o[j][i-1]
                gas.DPX = array_rho[j][i], array_p[j][i], x_post
                array_s[j][i] = gas.entropy_mass
                ### count
                array_a_fr[j][i] = soundspeed_fr(gas)
                array_M[j][i] = array_V[j][i] / array_a_fr[j][i]
                array_alpha[j][i] = np.arcsin(1./array_M[j][i])
                ### delta_c
                theta_3_new = array_theta_3[j][i-1]
                delta_c = abs((theta_3-theta_3_new)/theta_3)
                theta_3 = theta_3_new


        elif j == j_bottom:
            delta_angle_bottom = 1.0
            while delta_angle_bottom >= eps_c:
                ### b_bottom
                ### =====================================================================
                ### predictor
                ### =====================================================================
                #####################################################################################################(c)
                ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
                array_lambda_minus[j-1][i] = np.tan(array_theta[j-1][i]-array_alpha[j-1][i])
                ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
                array_Q_minus[j-1][i] = np.sqrt((array_M[j-1][i])**2.-1.) / (array_rho[j-1][i]*array_V[j-1][i]**2.)
                #####################################################################################################(d)
                # ### eq17dot44_eq17dot45
                ### eq17dot44_eq17dot45--------------------------------------------------------------------------------------------------------------------変更（for wall）
                array_x[j][i], array_y[j][i] = func_cross_gas_dynamics(\
                    (array_x[j-1][i], array_y[j-1][i]),\
                        (array_x[j][i-1], array_y[j][i-1]),\
                            array_lambda_minus[j-1][i], \
                                np.tan(angle_bottom))           ### ================= 条件によって変更する壁面境界条件
                ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
                array_T_minus[j-1][i] = array_Q_minus[j-1][i] * array_p[j-1][i] - array_theta[j-1][i]
                #####################################################################################################(e)
                ### eq17dot49 (the modified euler predictor-corrector)
                ### eq17dot43_eq17dot46
                ### state[-1][i-1] => state3[-1][i]
                array_x_3[j][i-1] = array_x[j][i-1]
                array_y_3[j][i-1] = array_y[j][i-1]
                array_theta_3[j][i-1] = array_theta[j][i-1]
                array_lambda_o[j][i-1] = array_lambda_o[j][i-1]
                array_p_3[j][i-1] = array_p[j][i-1]
                array_rho_3[j][i-1] = array_rho[j][i-1]
                array_V_3[j][i-1] = array_V[j][i-1]
                #####################################################################################################(f)
                gas.SPX = s_post, array_p_3[j][i-1], x_post
                array_a_fr_3[j][i-1] = soundspeed_fr(gas)
                ### eq.(g) & (h) for calculating p4, theta4
                array_theta[j][i] = angle_bottom
                array_p[j][i] = (array_T_minus[j-1][i]+array_theta[j][i]) / array_Q_minus[j-1][i] ###-------------（変更）
                # ### eq.(i) & (j) for calculating V4, rho4
                # gas.SPX = s_post, array_p[j][i], x_post
                # array_V[j][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                # array_rho[j][i] = gas.density_mass
                ### eq.(i) & (j) for calculating V4, rho4
                array_V[j][i] = (array_T_o1[j][i-1]-array_p[j][i]) / array_R_o[j][i-1]
                array_rho[j][i] = (array_p[j][i]-array_T_o2[j][i-1]) / array_A_o[j][i-1]
                gas.DPX = array_rho[j][i], array_p[j][i], x_post
                array_s[j][i] = gas.entropy_mass
                ### set predictor (state4)
                p_4 = array_p[j][i]
                delta_c = 1.0
                ### =====================================================================
                ### corrector : 全て入れなおせているのだろうか？
                ### =====================================================================
                while delta_c >= eps_c:
                    #####################################################################################################(g)
                    ### along Mach line 24 (C+)
                    ### along Mach line 14 (C-)
                    array_p_minus[j-1][i] = (array_p[j-1][i] + array_p[j][i]) /2.
                    array_theta_minus[j-1][i] = (array_theta[j-1][i] + array_theta[j][i]) /2.
                    gas.SPX = s_post, array_p_minus[j-1][i], x_post
                    array_V_minus[j-1][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                    array_rho_minus[j-1][i] = gas.density_mass
                    array_y_minus[j-1][i] = (array_y[j-1][i] + array_y[j][i]) /2.
                    array_a_fr_minus[j-1][i] = soundspeed_fr(gas)
                    array_M_minus[j-1][i] = array_V_minus[j-1][i] / array_a_fr_minus[j-1][i]
                    array_alpha_minus[j-1][i] = np.arcsin(1./array_M_minus[j-1][i])
                    array_lambda_minus[j-1][i] = np.tan(array_theta_minus[j-1][i]-array_alpha_minus[j-1][i])
                    array_Q_minus[j-1][i] = np.sqrt(array_M_minus[j-1][i]**2.-1.) / (array_rho_minus[j-1][i]*array_V_minus[j-1][i]**2.)
                    #####################################################################################################(h)
                    ### eq17dot44_eq17dot45-------------------------------------------------------------------変更（for wall）
                    array_x[j][i], array_y[j][i] = func_cross_gas_dynamics(\
                        (array_x[j-1][i], array_y[j-1][i]),\
                            (array_x[j][i-1], array_y[j][i-1]),\
                                array_lambda_minus[j-1][i], \
                                    np.tan(angle_bottom))
                    ### T+ & T- - eq17dot52_eq17dot53 (first step predictor
                    array_T_minus[j-1][i] = -array_S_minus[j-1][i] * (array_x[j][i]-array_x[j-1][i]) + \
                        array_Q_minus[j-1][i] * array_p[j-1][i] - array_theta[j-1][i]
                    #####################################################################################################(i)
                    ### state[-1][i-1] => state3[-1][i]
                    array_x_3[j][i-1] = array_x[j][i-1]
                    array_y_3[j][i-1] = array_y[j][i-1]
                    array_theta_3[j][i-1] = array_theta[j][i-1]
                    array_lambda_o[j][i-1] = array_lambda_o[j][i-1]
                    array_p_3[j][i-1] = array_p[j][i-1]
                    array_rho_3[j][i-1] = array_rho[j][i-1]
                    array_V_3[j][i-1] = array_V[j][i-1]
                    #####################################################################################################(j)
                    ### (start) ここで計算が「predictor」と「corrector」で異なる
                    array_p_o[j][i-1] = (array_p_3[j][i-1] + array_p[j][i]) / 2.
                    gas.SPX = s_post, array_p_o[j][i-1], x_post
                    array_rho_o[j][i-1] = gas.density_mass
                    array_V_o[j][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                    ### (end) 
                    array_a_fr_3[j][i-1] = soundspeed_fr(gas)
                    array_R_o[j][i-1] = array_rho_3[j][i-1] * array_V_3[j][i-1]
                    array_A_o[j][i-1] = array_a_fr_3[j][i-1] ** 2.
                    array_T_o1[j][i-1] = array_R_o[j][i-1] * array_V_3[j][i-1] + array_p_3[j][i-1] # using state3
                    array_T_o2[j][i-1] = array_p_3[j][i-1] - array_A_o[j][i-1] * array_rho_3[j][i-1] # using state3
                    ### eq.(g) & (h) for calculating p4, theta4
                    array_theta[j][i] = angle_bottom                    ### ================= 条件によって変更する壁面境界条件
                    array_p[j][i] = (array_T_minus[j-1][i]+array_theta[j][i]) / array_Q_minus[j-1][i]
                    # ### eq.(i) & (j) for calculating V4, rho4
                    # gas.SPX = s_post, array_p[-1][i], x_post
                    # array_rho[j][i] = gas.density_mass
                    # array_V[j_bottom][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                    # array_a_fr[j][i] = soundspeed_fr(gas)
                    # array_M[j][i] = array_V[j][i] / array_a_fr[j][i]
                    # array_alpha[j][i] = np.arcsin(1./array_M[j][i])
                    ### eq.(i) & (j) for calculating V4, rho4
                    array_V[j][i] = (array_T_o1[j][i-1]-array_p[j][i]) / array_R_o[j][i-1]
                    array_rho[j][i] = (array_p[j][i]-array_T_o2[j][i-1]) / array_A_o[j][i-1]
                    gas.DPX = array_rho[j][i], array_p[j][i], x_post
                    array_s[j][i] = gas.entropy_mass
                    ### delta_c
                    p_4_new = array_p[j][i]
                    delta_c = abs((p_4-p_4_new)/p_4)
                    p_4 = p_4_new
                ### M1=1 と仮定して計算（繰り返し計算なし）
                ### M1=1 の P2 とarray_p[-1][i] を比較
                ###### (i)  choked inflow -> no further calculation
                ###### (ii) unchoked inflow -> iteration until P1 = P2
                A1_over_A3 = 0.2
                P3 = array_p[j][i] ### point 4
                ### 現在の列の底面の要素
                ### mode_bottom == 0 : no inflow
                ### mode_bottom == 1 : start point of inflow
                ### mode_bottom == 2 : set initial characteristic lines under slip line
                ### mode_bottom == 3 : calculate interior characteristic lines under slip line
                
                if array_p[j][i] > P_ple and mode_bottom == 0: ### no inflow
                    # angle_bottom = angle_bottom ### stay
                    delta_angle_bottom = eps_c * 0.99 ### 再計算なし
                    mode_bottom = 0 ### mode_bottom の変化なし
                    
                elif array_p[j][i] <= P_ple and mode_bottom == 0: ### 未燃混合気層の計算「1」層目の計算開始，ただの点
                    # angle_bottom = angle_bottom ### stay
                    
                    x_1 = array_x[j_bottom][i]
                    y_1 = array_y[j_bottom][i]
                    p_1 = array_p[j_bottom][i]

                    print('subsonic /// initial_point at point 1')
                    M1 = 0.8 ### predictor of Mach_number
                    P1 = P_ple * (1.+(gamma_fr_ple-1.)/2.*M1) ** (-gamma_fr_ple/(gamma_fr_ple-1.)) * 2. ### predictor であればなんでも良いのか？？？
                    delta_P1 = 1.0
                    P3 = p_1 ### 内挿した p_13 で inflow_boudary の jump_relation 計算
                    while delta_P1 >= eps_c:
                        mdot_over_A1 = P_ple / np.sqrt(T_ple) * np.sqrt(gamma_fr_ple/R_ple) * M1 * \
                            (1.+(gamma_fr_ple-1.)/2.*M1**2.)**(-(gamma_fr_ple+1.)/(2.*(gamma_fr_ple-1.)))
                        v3 = func_kainokoushiki(1./2., gamma_fr_ple/(gamma_fr_ple-1.)*P3/(A1_over_A3*mdot_over_A1), -gamma_fr_ple/(gamma_fr_ple-1.)*P_ple*rho_ple) ### lab_fixed 
                        gas.SPX = s_ple, P1, q
                        v1 = np.sqrt(2.0*(h_ple - gas.enthalpy_mass)) ### lab_fixed                   
                        a_fr_1 = soundspeed_fr(gas)
                        M1 = v1 / a_fr_1 ### lab_fixed 
                        P2 = (mdot_over_A1 * (v3 - v1) - P1 + P3 / A1_over_A3) / (1./A1_over_A3 - 1.) ### P2 in an article
                        delta_P1 = abs((P1-P2)/P2)
                        P1 = P2
                        if M1 >= 1.:
                            print('ERROR : 亜音速なんですけどおかしくないですか？')
                    del P3 ### 以降のエラー防止のための試み
                    
                    h_1 = h_ple - v3**2./2. ### static enthalpy lab_fixed
                    gas.HPX = h_1, p_1, q ### lab_fixed
                    s_1 = gas.entropy_mass ### both lab_fixed and shock_fixed, この後エントロピーの分布が生まれるってことやんね？どうせ斜め衝撃波背後の領域は entropy 分布できる
                    rho_1 = gas.density_mass ### both lab_fixed and shock_fixed
                    a_fr_1 = soundspeed_fr(gas)
                    
                    ### ここの値次第で entropy の変化を考慮すべきかどうかを判断する
                    print('s_ple =', s_ple, '/// s_13 =', s_1, '/// delta_s =', (s_ple-s_1)/s_1, '%')
                    
                    ### 
                    ### shock_fixed
                    ### theta_1 = np.arctan(v3/CJ_speed)
                    theta_1 = np.arctan(v3/CJ_speed)
                    V_1 = np.sqrt(v3**2.+CJ_speed**2.)
                    # h_t_1_shock_fixed = h_1 + V_1**2./2. ### 意味のない計算
                    h_t_shock_fixed = h_ple + CJ_speed**2. ### 全ての流線の衝撃波固定座標系での全エンタルピーはこれだよね？ h_ple+CJ_speed**2./2.
                    M_1 = V_1 / a_fr_1
                    alpha_1 = np.arcsin(1./M_1)
                    
                    ### delta_theta 更新
                    delta_angle_bottom = abs((array_theta[j_bottom][i]-theta_1)/theta_1)
                                        
                    if delta_angle_bottom <= eps_c:
                        print('mode_bottom : 0 --->>> 1')
                        mode_bottom = 1 ### mode_bottom : 0 -> 1（inflow有のモードに移行
                    
                elif array_p[j][i] <= P_ple and mode_bottom == 1: ### 未燃混合気層の計算「2」層目の計算開始，初期値の特性線の設定
                    ### 既燃ガス側の array_theta[j][i] を 既燃ガス側の array_theta[j][i-1] として計算（仮定） 
                    ### 既燃ガス側の array_p[j][i] と未燃ガス側の圧力が一致するように計算
                    ### 未燃ガス側の p, theta を計算　-> 
                    ### 既燃ガス側の array_theta[j][i] を 未燃ガス側の theta として計算（仮定）
                    ### a いらない？？？ delta_angle_bottom だけで操作可能では？？？
                    ### free boundary unit process
                    ### point_3 predictor
                    array_x_fm[-1][0] = array_x[j_bottom][i]*1.1 ### p_3
                    array_y_fm[-1][0] = array_y[j_bottom][i]*1.1 ### p_3
                    array_p_fm[-1][0] = array_p[j_bottom][i]*0.9 ### p_3
                    p_3 = array_p_fm[-1][0]
                    delta_p_3 = 1.0
                    while delta_p_3 >= eps_c:
                        ### point between 1-3 for 特性線 C+
                        ### (x, y, p) のみ point 1 & 2 の内挿
                        ### それらの値を使って inflow 繰り返し計算を行った
                        x_13 = (x_1+array_x_fm[-1][0])/2.
                        y_13 = (y_1+array_y_fm[-1][0])/2.
                        p_13 = (p_1+array_p_fm[-1][0])/2.
                        print('subsonic /// initial_point between 1-3')
                        M1 = 0.8 ### predictor of Mach_number
                        P1 = P_ple * (1.+(gamma_fr_ple-1.)/2.*M1) ** (-gamma_fr_ple/(gamma_fr_ple-1.)) * 2. ### predictor であればなんでも良いのか？？？
                        delta_P1 = 1.0
                        P3 = p_13 ### 内挿した p_13 で inflow_boudary の jump_relation 計算
                        while delta_P1 >= eps_c:
                            mdot_over_A1 = P_ple / np.sqrt(T_ple) * np.sqrt(gamma_fr_ple/R_ple) * M1 * \
                                (1.+(gamma_fr_ple-1.)/2.*M1**2.)**(-(gamma_fr_ple+1.)/(2.*(gamma_fr_ple-1.)))
                            v3 = func_kainokoushiki(1./2., gamma_fr_ple/(gamma_fr_ple-1.)*P3/(A1_over_A3*mdot_over_A1), -gamma_fr_ple/(gamma_fr_ple-1.)*P_ple*rho_ple) ### lab_fixed 
                            gas.SPX = s_ple, P1, q
                            v1 = np.sqrt(2.0*(h_ple - gas.enthalpy_mass)) ### lab_fixed                   
                            a_fr_1 = soundspeed_fr(gas)
                            M1 = v1 / a_fr_1 ### lab_fixed 
                            P2 = (mdot_over_A1 * (v3 - v1) - P1 + P3 / A1_over_A3) / (1./A1_over_A3 - 1.) ### P2 in an article
                            delta_P1 = abs((P1-P2)/P2)
                            P1 = P2
                            if M1 >= 1.:
                                print('ERROR : 亜音速なんですけどおかしくないですか？')
                        del P3 ### 以降のエラー防止のための試み
                        h_13 = h_ple - v3**2./2. ### static enthalpy lab_fixed
                        gas.HPX = h_13, p_13, q ### lab_fixed
                        s_13 = gas.entropy_mass ### both lab_fixed and shock_fixed, この後エントロピーの分布が生まれるってことやんね？どうせ斜め衝撃波背後の領域は entropy 分布できる
                        rho_13 = gas.density_mass ### both lab_fixed and shock_fixed
                        a_fr_13 = soundspeed_fr(gas)
                        ### ここの値次第で entropy の変化を考慮すべきかどうかを判断する
                        print('s_ple =', s_ple, '/// s_13 =', s_13, '/// delta_s =', (s_ple-s_13)/s_13, '%')
                        ### 
                        ### shock_fixed
                        ### theta_13 = np.arctan(v3/CJ_speed)
                        theta_13 = np.arctan(v3/CJ_speed)
                        V_13 = np.sqrt(v3**2.+CJ_speed**2.)
                        # h_t_13_shock_fixed = h_13 + V_13**2./2. ### 意味のない計算 /// 全ての流線の衝撃波固定座標系での全エントロピーは h_ple+CJ_speed**2./2.
                        M_13 = V_13 / a_fr_13
                        alpha_13 = np.arcsin(1./M_13)
                        ###
                        ### point_1(Co) & point_1-3(C+) -> theta_4 (point_2)
                        ### あくまで point_2 の predictor
                        #####################################################################################################(e)
                        #####################################################################################################(f)
                        ### delta_theta_4
                        array_p_fm[0][0] = array_p[j_bottom][i] ### point_2 == array_[j_bottom][i]
                        array_s_fm[0][0] = s_13 ### predictor なのでなんでもいい？
                        gas.SPX = array_s_fm[0][0], array_p_fm[0][0], q ### predictor なのでなんでもいい？
                        array_rho_fm[0][0] = gas.density_mass
                        array_V_fm[0][0] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                        array_a_fr_fm[0][0] = soundspeed_fr(gas)
                        array_M_fm[0][0] = array_V_fm[0][0] / array_a_fr_fm[0][0]
                        array_alpha_fm[0][0] = np.arcsin(1./array_M_fm[0][0])
                        #####################################################################################################(c)
                        ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
                        lambda_plus_13 = np.tan(theta_13+alpha_13)
                        ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
                        Q_plus_13 = np.sqrt((M_13)**2.-1.) / (rho_13*V_13**2.)
                        #####################################################################################################(d)
                        ### eq17dot43_eq17dot44
                        lambda_o_1 = np.tan((theta_1))
                        array_x_fm[0][0], array_y_fm[0][0] = func_cross_gas_dynamics(\
                            (x_1, y_1),\
                                (x_13, y_13),\
                                    lambda_o_1, \
                                        lambda_plus_13)
                        ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
                        S_plus_13 = 0 ### planar-flow
                        T_plus_13 = -S_plus_13 * (array_x_fm[0][0]-x_13) + \
                            Q_plus_13 * p_13 + theta_13
                        array_theta_fm[0][0] = T_plus_13-Q_plus_13 * array_p_fm[0][0]
                        ### set predictor
                        theta_4 = array_theta_fm[0][0]
                        delta_theta_4 = 1.0
                        while delta_theta_4 >= eps_c:
                            ### along Mach line 24 (C+)
                            p_plus_13 = (p_13 + array_p_fm[0][0]) /2.
                            theta_plus_13 = (theta_13 + array_theta_fm[0][0]) /2.
                            s_plus_13 = (s_13 + array_s_fm[0][0]) /2.
                            gas.SPX = s_plus_13, p_plus_13, q
                            if h_t_shock_fixed - gas.enthalpy_mass < 0:
                                print('ERROR : h_t_13_shock_fixed が小さいぞおい！')
                            V_plus_13 = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                            rho_plus_13 = gas.density_mass
                            y_plus_13 = (y_13 + array_y_fm[0][0]) /2.
                            a_fr_plus_13 = soundspeed_fr(gas)
                            M_plus_13 = V_plus_13 / a_fr_plus_13
                            alpha_plus_13 = np.arcsin(1./M_plus_13)
                            #####################################################################################################(c)
                            ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
                            lambda_plus_13 = np.tan(theta_plus_13+alpha_plus_13)
                            ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
                            Q_plus_13 = np.sqrt((M_plus_13)**2.-1.) / (rho_plus_13*V_plus_13**2.)
                            #####################################################################################################(d)
                            ### eq17dot43_eq17dot44
                            lambda_o_1 = np.tan((theta_1+array_theta_fm[0][0])/2.)
                            array_x_fm[0][0], array_y_fm[0][0] = func_cross_gas_dynamics(\
                                (x_1, y_1),\
                                    (x_13, y_13),\
                                        lambda_o_1, \
                                            lambda_plus_13)
                            ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
                            S_plus_13 = 0 ### planar-flow
                            T_plus_13 = -S_plus_13 * (array_x_fm[0][0]-x_13) + \
                                Q_plus_13 * p_13 + theta_13
                            array_theta_fm[0][0] = T_plus_13-Q_plus_13 * array_p_fm[0][0]#######################################
                            theta_4_new = array_theta_fm[0][0]
                            delta_theta_4 = abs((theta_4-theta_4_new)/theta_4)
                            theta_4 = theta_4_new
                        ### point_3 の計算開始
                        ### array_x_fm[-1][0] 更新
                        ### array_y_fm[-1][0] 更新
                        ### array_p_fm[-1][0] 更新
                        print('subsonic /// initial_point at point_3')
                        M1 = 0.8 ### predictor of Mach_number
                        P1 = P_ple * (1.+(gamma_fr_ple-1.)/2.*M1) ** (-gamma_fr_ple/(gamma_fr_ple-1.)) * 2. ### predictor であればなんでも良いのか？？？
                        delta_P1 = 1.0
                        P3 = array_p_fm[-1][0] ### 内挿した p_13 で inflow_boudary の jump_relation 計算
                        while delta_P1 >= eps_c:
                            mdot_over_A1 = P_ple / np.sqrt(T_ple) * np.sqrt(gamma_fr_ple/R_ple) * M1 * \
                                (1.+(gamma_fr_ple-1.)/2.*M1**2.)**(-(gamma_fr_ple+1.)/(2.*(gamma_fr_ple-1.)))
                            v3 = func_kainokoushiki(1./2., gamma_fr_ple/(gamma_fr_ple-1.)*P3/(A1_over_A3*mdot_over_A1), -gamma_fr_ple/(gamma_fr_ple-1.)*P_ple*rho_ple) ### lab_fixed 
                            gas.SPX = s_ple, P1, q
                            v1 = np.sqrt(2.0*(h_ple - gas.enthalpy_mass)) ### lab_fixed                   
                            a_fr_1 = soundspeed_fr(gas)
                            M1 = v1 / a_fr_1 ### lab_fixed 
                            P2 = (mdot_over_A1 * (v3 - v1) - P1 + P3 / A1_over_A3) / (1./A1_over_A3 - 1.) ### P2 in an article
                            delta_P1 = abs((P1-P2)/P2)
                            P1 = P2
                            if M1 >= 1.:
                                print('ERROR : 亜音速なんですけどおかしくないですか？')
                        del P3 ### 以降のエラー防止のための試み
                        array_h_fm_lab_fixed[-1][0] = h_ple - v3**2./2. ### static enthalpy lab_fixed
                        gas.HPX = array_h_fm_lab_fixed[-1][0], array_p_fm[-1][0], q ### lab_fixed
                        array_s_fm[-1][0] = gas.entropy_mass ### both lab_fixed and shock_fixed, この後エントロピーの分布が生まれるってことやんね？どうせ斜め衝撃波背後の領域は entropy 分布できる
                        array_rho_fm[-1][0] = gas.density_mass ### both lab_fixed and shock_fixed
                        array_a_fr_fm[-1][0] = soundspeed_fr(gas)
                        ### ここの値次第で entropy の変化を考慮すべきかどうかを判断する
                        print('s_ple =', s_ple, '/// array_s_fm[-1][0] =', array_s_fm[-1][0], '/// delta_s =', (s_ple-array_s_fm[-1][0])/array_s_fm[-1][0], '%')
                        ### 
                        ### shock_fixed
                        ### theta_13 = np.arctan(v3/CJ_speed)
                        array_theta_fm[-1][0] = np.arctan(v3/CJ_speed)
                        array_V_fm[-1][0] = np.sqrt(v3**2.+CJ_speed**2.)
                        # array_h_fm_shock_fixed[-1][0] = array_h_fm_lab_fixed[-1][0] + array_V_fm[-1][0]**2./2. 意味のない計算
                        array_M_fm[-1][0] = array_V_fm[-1][0] / array_a_fr_fm[-1][0]
                        array_alpha_fm[-1][0] = np.arcsin(1./array_M_fm[-1][0])
                        ###
                        ### 特性線 C- の計算
                        p_minus_23 = (array_p_fm[0][0]+array_p_fm[-1][0])/2.
                        s_minus_23 = (array_s_fm[0][0]+array_s_fm[-1][0])/2.
                        gas.SPX = s_minus_23,p_minus_23,q
                        V_minus_23 = np.sqrt(2.0*(h_t_shock_fixed-gas.enthalpy_mass))
                        rho_minus_23 = gas.density_mass
                        a_fr_minus_23 = soundspeed_fr(gas)
                        M_minus_23 = V_minus_23 / a_fr_minus_23
                        Q_minus_23 = np.sqrt((M_minus_23)**2.-1.) / (rho_minus_23*V_minus_23**2.)
                        T_minus_23 = Q_minus_23*p_1-theta_1
                        array_p_fm[-1][0] = (T_minus_23+array_theta_fm[-1][0])/Q_minus_23
                        ### delta_p_3 更新
                        delta_p_3 = abs((p_3-array_p_fm[-1][0])/p_3)
                        p_3 = array_p_fm[-1][0]

                    ### delta_theta 更新
                    ### array_theta[j_bottom][i] & point_2 の比較 /// 圧力と流線の傾きが等しい
                    delta_angle_bottom = abs((array_theta[j_bottom][i]-array_theta_fm[0][0])/array_theta_fm[0][0])
                                        
                    if delta_angle_bottom <= eps_c:
                        print('mode_bottom : 1 --->>> 2')
                        mode_bottom = 2 ### mode_bottom : 1 -> 2（inflow有のモードに移行
                        
                        ### 特性線の初期値の設定
                        ### array_[0][0] ~ array[-1][0] の間を埋めていく
                        array_x_23 = np.linspace(array_x[0][0],array_x[-1][0],num_ch_fm_tate)
                        array_y_23 = np.linspace(array_y[0][0],array_y[-1][0],num_ch_fm_tate)
                        array_theta_23 = np.linspace(array_theta[0][0],array_theta[-1][0],num_ch_fm_tate)
                        array_M_23 = np.linspace(array_M[0][0],array_M[-1][0],num_ch_fm_tate)
                        
                        for i0 in range(num_ch_fm_tate):
                            array_x_fm[i0][0] = array_x_23[i0]
                            array_y_fm[i0][0] = array_y_23[i0]
                            array_theta_fm[i0][0] = array_theta_23[i0]
                            array_M_fm[i0][0] = array_M_23[i0]
                            array_p_fm[i0][0], \
                                array_t_fm[i0][0], \
                                    array_R_fm[i0][0], \
                                        array_rho_fm[i0][0], \
                                            array_a_fr_fm[i0][0], \
                                                array_V_fm[i0][0], \
                                                    array_gamma_fm[i0][0] = func_M2P(array_M_fm[i0][0])
                            array_neu_fm[i0][0] = func_M2neu(array_M_fm[i0][0], array_gamma_fm[i0][0])
                            array_alpha_fm[i0][0] = np.arcsin(1./array_M_fm[i0][0])
                    elif delta_angle_bottom > eps_c:
                        mode_bottom = 1
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                 
                        
                elif array_p[j][i] <= P_ple and mode_bottom == 2: ### 未燃混合気層の計算「3」層目以降の計算開始
                    
                    ### p = f(x) : 更新
                    ### array_p[-1][i-1] & array_p[-1][i] で内挿
                    ### 仮で x 座標を基準に内挿した関数を設定：p = f(x)
                    ### 仮で"区分的 3 次エルミート補間"　interpolate.PchipInterpolator
                    x_observed = np.array([array_x[j][i-1],array_x[j][i]])
                    p_observed = np.array([array_p[j][i-1],array_p[j][i]])
                    func_x_vs_p = interpolate.PchipInterpolator(x_observed, p_observed)
                    
                    delta_x_koshi = 0
                    j_bottom_fm = num_ch_fm_tate-1
                    
                    ### もしも free boundary condition に隣接する要素の x 座標が array_x[j_bottom][i] を超えたら停止
                    ### free boundary condition の要素の計算 ///  x 座標が array_x[j_bottom][i] を超えたら停止信号（delta_x_koshi = 1）
                    # i_fm = 0 ### もっと前の階層で更新しなければならない
                    while delta_x_koshi == 0:
                        i_fm += 1
                        for j_fm in range(num_ch_fm_tate):
                            if j_fm == 0:
                                #####################################################################################################(e)
                                #####################################################################################################(f)
                                ### delta_theta_4
                                ### p = f(x) なので繰り返し計算で (x, p) を計算する必要がある？
                                delta_x = 1.0
                                x_old = array_x_fm[j_fm][i_fm-1] ### predictor
                                while delta_x >= eps_c:
                                    array_p_fm[j_fm][i_fm] = func_x_vs_p(x_old) ### 滑り面の圧力の設定
                                    array_s_fm[j_fm][i_fm] = array_s_fm[j_fm][i_fm-1] ### predictor なのでなんでもいい？，滑り面（流線）に沿って一定？
                                    gas.SPX = array_s_fm[j_fm][i_fm], array_p_fm[j_fm][i_fm], q ### predictor なのでなんでもいい？
                                    array_rho_fm[j_fm][i_fm] = gas.density_mass
                                    array_V_fm[j_fm][i_fm] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    array_a_fr_fm[j_fm][i_fm] = soundspeed_fr(gas)
                                    array_M_fm[j_fm][i_fm] = array_V_fm[j_fm][i_fm] / array_a_fr_fm[j_fm][i_fm]
                                    array_alpha_fm[j_fm][i_fm] = np.arcsin(1./array_M_fm[j_fm][i_fm])
                                    #####################################################################################################(c)
                                    ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
                                    array_lambda_plus_fm[j_fm+1][i_fm-1] = np.tan(array_theta_fm[j_fm+1][i_fm-1]+array_alpha_fm[j_fm+1][i_fm-1])
                                    ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
                                    array_Q_plus_fm[j_fm+1][i_fm-1] = np.sqrt((array_M_fm[j_fm][i_fm])**2.-1.) / (array_rho_fm[j_fm][i_fm]*array_V_fm[j_fm][i_fm]**2.)
                                    #####################################################################################################(d)
                                    ### eq17dot43_eq17dot44
                                    array_lambda_o_fm[j_fm][i_fm-1] = np.tan(array_theta_fm[j_fm][i_fm-1])
                                    array_x_fm[j_fm][i_fm], array_y_fm[j_fm][i_fm] = func_cross_gas_dynamics(\
                                        (array_x_fm[j_fm][i_fm-1], array_y_fm[j_fm][i_fm-1]),\
                                            (array_x_fm[j_fm+1][i_fm-1], array_y_fm[j_fm+1][i_fm-1]),\
                                                array_lambda_o_fm[j_fm][i_fm-1], \
                                                    array_lambda_plus_fm[j_fm+1][i_fm-1])
                                    delta_x = abs((x_old - array_x_fm[j_fm][i_fm])/x_old)
                                    x_old = array_x_fm[j_fm][i_fm]
                                ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
                                array_T_plus_fm[j_fm+1][i_fm-1] = array_Q_plus_fm[j_fm+1][i_fm-1] * array_p_fm[j_fm+1][i_fm-1] + array_theta_fm[j_fm+1][i_fm-1]
                                array_theta_fm[j_fm][i_fm] = array_T_plus_fm[j_fm+1][i_fm-1]-array_Q_plus_fm[j_fm+1][i_fm-1]* array_p_fm[j_fm][i_fm]
                                ### set predictor
                                theta_4 = array_theta_fm[j_fm][i_fm]
                                delta_theta_4 = 1.0
                                while delta_theta_4 >= eps_c:
                                    ### along Mach line 24 (C+)
                                    array_p_plus_fm[j_fm+1][i_fm-1] = (array_p_fm[j_fm+1][i_fm-1] + array_p_fm[j_fm][i_fm]) /2.
                                    array_theta_plus_fm[j_fm+1][i_fm-1] = (array_theta_fm[j_fm+1][i_fm-1] + array_theta_fm[j_fm][i_fm]) /2.
                                    array_s_plus_fm[j_fm+1][i_fm-1] = (array_s_fm[j_fm+1][i_fm-1] + array_s_fm[j_fm][i_fm]) /2.
                                    gas.SPX = array_s_plus_fm[j_fm+1][i_fm-1], array_p_plus_fm[j_fm+1][i_fm-1], q
                                    if h_t_shock_fixed  - gas.enthalpy_mass < 0:
                                        print('ERROR : h_t_13_shock_fixed が小さいぞおい！')
                                    array_V_plus_fm[j_fm+1][i_fm-1] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    array_rho_plus_fm[j_fm+1][i_fm-1] = gas.density_mass
                                    array_y_plus_fm[j_fm+1][i_fm-1] = (array_y_fm[j_fm+1][i_fm-1] + array_y_fm[j_fm][i_fm]) /2.
                                    array_a_fr_plus_fm[j_fm+1][i_fm-1] = soundspeed_fr(gas)
                                    array_M_plus_fm[j_fm+1][i_fm-1] = array_V_plus_fm[j_fm+1][i_fm-1] / array_a_fr_plus_fm[j_fm+1][i_fm-1]
                                    array_alpha_plus_fm[j_fm+1][i_fm-1] = np.arcsin(1./array_M_plus_fm[j_fm+1][i_fm-1])
                                    #####################################################################################################(c)
                                    ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
                                    array_lambda_plus_fm[j_fm+1][i_fm-1] = np.tan(array_theta_plus_fm[j_fm+1][i_fm-1]+array_alpha_plus_fm[j_fm+1][i_fm-1])
                                    ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
                                    array_Q_plus_fm[j_fm+1][i_fm-1] = np.sqrt((array_M_plus_fm[j_fm+1][i_fm-1])**2.-1.) / \
                                        (array_rho_plus_fm[j_fm+1][i_fm-1]*array_V_plus_fm[j_fm+1][i_fm-1]**2.)
                                    #####################################################################################################(d)
                                    ### eq17dot43_eq17dot44
                                    array_lambda_o_fm[j_fm][i_fm-1] = np.tan((array_theta_fm[j_fm][i_fm-1]+array_theta_fm[j_fm][i_fm])/2.)
                                    array_x_fm[j_fm][i_fm], array_y_fm[j_fm][i_fm] = func_cross_gas_dynamics(\
                                        (array_x_fm[j_fm][i_fm-1], array_y_fm[j_fm][i_fm-1]),\
                                            (array_x_fm[j_fm+1][i_fm-1], array_y_fm[j_fm+1][i_fm-1]),\
                                                array_lambda_o_fm[j_fm][i_fm-1], \
                                                    array_lambda_plus_fm[j_fm+1][i_fm-1])
                                    ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
                                    array_T_plus_fm[j_fm+1][i_fm-1] = array_Q_plus_fm[j_fm+1][i_fm-1] * array_p_fm[j_fm+1][i_fm-1] + array_theta_fm[j_fm+1][i_fm-1]
                                    array_theta_fm[j_fm][i_fm] = array_T_plus_fm[j_fm+1][i_fm-1]-array_Q_plus_fm[j_fm+1][i_fm-1] * array_p_fm[j_fm][i_fm]##########
                                    theta_4_new = array_theta_fm[j_fm][i_fm]
                                    delta_theta_4 = abs((theta_4-theta_4_new)/theta_4)
                                    theta_4 = theta_4_new
                            
                            
                                ### 計算を続ける x 座標を判定する
                                if array_x_fm[j_fm][i_fm] <= array_x[j][i]: ### array_x_fm[0][i_fm] の話
                                    delta_x_koshi = 1 ### 計算を停止 -> inversed_unit_process へ移行
                                    ### ===========================================================================
                                    ### ===========================================================================
                                    ### inversed_unit_process
                                    ### point_3 : array_[0][i_fm-1] /// 現在の i_fm はすでに array_x[j][i] を超えたときの番号
                                    ### point_1 : array_[1][i_fm-1] /// 現在の i_fm はすでに array_x[j][i] を超えたときの番号
                                    ### point_2 : 保存しないかもしれない (_iv_2)
                                    ### point_4 : array_x[j][i], array_y[j][i], array_theta[j][i] -->> array_p[j][i]
                                    ### ===========================================================================
                                    ### ===========================================================================
                                    
                                    ### d_inversed_unit_process
                                    ### =====================================================================
                                    ### predictor
                                    ### =====================================================================
                                    ### Mach_line 24 -> _plus_2
                                    p_iv_2 = (array_p_fm[j_fm][i_fm-1]+array_p_fm[j_fm+1][i_fm-1])/2.
                                    s_iv_2 = (array_s_fm[j_fm][i_fm-1]+array_s_fm[j_fm+1][i_fm-1])/2.
                                    theta_iv_2 = (array_theta_fm[j_fm][i_fm-1]+array_theta_fm[j_fm+1][i_fm-1])/2.
                                    gas.SPX = s_iv_2, p_iv_2, q
                                    V_iv_2 = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    rho_iv_2 = gas.density_mass
                                    a_fr_iv_2 = soundspeed_fr(gas)
                                    M_iv_2 = V_iv_2 / a_fr_iv_2
                                    alpha_iv_2 = np.arcsin(1./M_iv_2)
                                    # x_iv_4 = array_x[j][i] ### まだ j & i は変更ないよね？？？
                                    y_iv_4 = array_y[j][i] ### まだ j & i は変更ないよね？？？
                                    theta_iv_4 = array_theta[j][i] ### まだ j & i は変更ないよね？？？
                                    theta_plus_iv_24 = (theta_iv_2+theta_iv_4)/2.
                                    lambda_plus_iv_24 = np.tan(theta_plus_iv_24+alpha_iv_2)
                                    ### （point 4 の傾きと座標）& 特性線 C- 1-3
                                    ### point 1 & lambda_minus
                                    ### point 4 & theta_4
                                    x_iv_2, y_iv_2 = func_cross_gas_dynamics(\
                                        (array_x[j][i], array_y[j][i]),\
                                            (array_x[j_fm+1][i_fm-1], array_y[j_fm+1][i_fm-1]),\
                                                lambda_plus_iv_24, \
                                                    array_lambda_minus[j_fm+1][i_fm-1])
                                    ### point 2 interpolated between 1 and 3
                                    delta_x_iv = 1.0
                                    x_iv_old = x_iv_2
                                    while delta_x_iv >= eps_c:
                                        p_iv_2 = array_p_fm[j_fm+1][i_fm-1] + (x_iv_2-array_x_fm[j_fm+1][i_fm-1])/(array_x_fm[j_fm][i_fm-1]-array_x_fm[j_fm+1][i_fm-1])*(array_p_fm[j_fm][i_fm-1]-array_p_fm[j_fm+1][i_fm-1])
                                        s_iv_2 = array_s_fm[j_fm+1][i_fm-1] + (x_iv_2-array_x_fm[j_fm+1][i_fm-1])/(array_x_fm[j_fm][i_fm-1]-array_x_fm[j_fm+1][i_fm-1])*(array_s_fm[j_fm][i_fm-1]-array_s_fm[j_fm+1][i_fm-1])
                                        theta_iv_2 = array_theta_fm[j_fm+1][i_fm-1] + (x_iv_2-array_x_fm[j_fm+1][i_fm-1])/(array_x_fm[j_fm][i_fm-1]-array_x_fm[j_fm+1][i_fm-1])*(array_theta_fm[j_fm][i_fm-1]-array_theta_fm[j_fm+1][i_fm-1])
                                        gas.SPX = s_iv_2, p_iv_2, q
                                        V_iv_2 = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                        rho_iv_2 = gas.density_mass
                                        a_fr_iv_2 = soundspeed_fr(gas)
                                        M_iv_2 = V_iv_2 / a_fr_iv_2
                                        alpha_iv_2 = np.arcsin(1./M_iv_2)
                                        theta_plus_iv_24 = (theta_iv_2+theta_iv_4)/2.
                                        lambda_plus_iv_24 = np.tan(theta_plus_iv_24+alpha_iv_2)
                                        y_plus_iv_2 = (y_iv_2+y_iv_4)/2.
                                        ### （point 4 の傾きと座標）& 特性線 C- 1-3
                                        ### point 1 & lambda_minus
                                        ### point 4 & theta_4
                                        x_iv_2, y_iv_2 = func_cross_gas_dynamics(\
                                            (array_x[j][i], array_y[j][i]),\
                                                (array_x[j_fm+1][i_fm-1], array_y[j_fm+1][i_fm-1]),\
                                                    lambda_plus_iv_24, \
                                                        array_lambda_minus[j_fm+1][i_fm-1])
                                    ### (c)
                                    Q_plus_iv_2 = np.sqrt(M_iv_2**2.-1.)/(rho_iv_2*V_iv_2**2.)
                                    T_plus_iv_2 = Q_plus_iv_2*p_iv_2+theta_iv_2
                                    ### (d) - stream line 34
                                    ### (e)
                                    p_iv_4 = (T_plus_iv_2 - theta_iv_4) / Q_plus_iv_2
                                    gas.SPX = s_1, p_iv_4, q ### 滑り面に沿って，流線に沿って，エントロピー一定 s_1 = const. #################################################################################変更する
                                    V_iv_4 = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    rho_iv_4 = gas.density_mass
                                    a_fr_iv_4 = soundspeed_fr(gas)
                                    M_iv_4 = V_iv_4 / a_fr_iv_4
                                    alpha_iv_4 = np.arcsin(1./M_iv_4)
                                    ### set predictor
                                    p_iv_4_old = p_iv_4
                                    delta_c = 1.0
                                    ### =====================================================================
                                    ### corrector : 全て入れなおせているのだろうか？
                                    ### ここで stream line 34 の間の値を計算して利用
                                    ### ここで 特性線 C+ 24 の間の値を計算して利用（average ）
                                    ### =====================================================================
                                    while delta_c >= eps_c:
                                        ### Mach_line 24 -> _plus_2
                                        delta_x_iv = 1.0
                                        x_iv_old = x_iv_2
                                        while delta_x_iv >= eps_c:
                                            p_iv_2 = array_p_fm[j_fm+1][i_fm-1] + (x_iv_2-array_x_fm[j_fm+1][i_fm-1])/(array_x_fm[j_fm][i_fm-1]-array_x_fm[j_fm+1][i_fm-1])*(array_p_fm[j_fm][i_fm-1]-array_p_fm[j_fm+1][i_fm-1])
                                            s_iv_2 = array_s_fm[j_fm+1][i_fm-1] + (x_iv_2-array_x_fm[j_fm+1][i_fm-1])/(array_x_fm[j_fm][i_fm-1]-array_x_fm[j_fm+1][i_fm-1])*(array_s_fm[j_fm][i_fm-1]-array_s_fm[j_fm+1][i_fm-1])
                                            theta_iv_2 = array_theta_fm[j_fm+1][i_fm-1] + (x_iv_2-array_x_fm[j_fm+1][i_fm-1])/(array_x_fm[j_fm][i_fm-1]-array_x_fm[j_fm+1][i_fm-1])*(array_theta_fm[j_fm][i_fm-1]-array_theta_fm[j_fm+1][i_fm-1])
                                            gas.SPX = s_iv_2, p_iv_2, q ### 滑り面に沿って，流線に沿って，エントロピー一定 s_1 = const. 
                                            V_iv_2 = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                            rho_iv_2 = gas.density_mass
                                            a_fr_iv_2 = soundspeed_fr(gas)
                                            M_iv_2 = V_iv_2 / a_fr_iv_2
                                            alpha_iv_2 = np.arcsin(1./M_iv_2)
                                            theta_plus_iv_24 = (theta_iv_2+theta_iv_4)/2.
                                            lambda_plus_iv_24 = np.tan(theta_plus_iv_24+alpha_iv_2)
                                            y_plus_iv_2 = (y_iv_2+y_iv_4)/2.
                                            ### （point 4 の傾きと座標）& 特性線 C- 1-3
                                            ### point 1 & lambda_minus
                                            ### point 4 & theta_4
                                            x_iv_2, y_iv_2 = func_cross_gas_dynamics(\
                                                (array_x[j][i], array_y[j][i]),\
                                                    (array_x[j_fm+1][i_fm-1], array_y[j_fm+1][i_fm-1]),\
                                                        lambda_plus_iv_24, \
                                                            array_lambda_minus[j_fm+1][i_fm-1])                                        
                                        
                                            y_iv_24 = (y_iv_2+array_y[j][i])/2.
                                            p_iv_24 = (p_iv_2+array_p[j][i])/2.
                                            gas.SPX = s_1, p_iv_24, q ### 滑り面に沿って，流線に沿って，エントロピー一定 s_1 = const. #################################################################################変更する
                                            V_iv_24 = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                            rho_iv_24 = gas.density_mass
                                            a_fr_iv_24 = soundspeed_fr(gas)
                                            M_iv_24 = V_iv_24 / a_fr_iv_24
                                            alpha_iv_24 = np.arcsin(1./M_iv_24)
                                            lambda_plus_iv_24 = np.tan(theta_plus_iv_24 + alpha_iv_24)
                                        
                                        Q_plus_iv_2 = np.sqrt(M_iv_24**2.-1.)/(rho_iv_24*V_iv_24**2.)
                                        T_plus_iv_2 = Q_plus_iv_2*p_iv_2+theta_iv_2
                                        
                                        ### (d) - stream line 34
                                        p_iv_34 = (array_p_fm[j_fm][i_fm-1]+array_p[j][i])/2.
                                        gas.SPX = s_1, p_iv_34, q ### 滑り面に沿って，流線に沿って，エントロピー一定 s_1 = const. 
                                        V_iv_34 = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                        rho_iv_34 = gas.density_mass
                                        a_fr_iv_34 = soundspeed_fr(gas)
                                        M_iv_34 = V_iv_34 / a_fr_iv_34
                                        alpha_iv_34 = np.arcsin(1./M_iv_34)
                                        
                                        ### (e)
                                        p_iv_4 = (T_plus_iv_2 - theta_iv_4) / Q_plus_iv_2

                                        ### delta_c
                                        p_iv_4_new = p_iv_4
                                        delta_c = abs((p_iv_4_old-p_iv_4_new)/p_iv_4_old)
                                        p_iv_4_old = p_iv_4_new

                                
 
                            elif delta_x_koshi == 0:
                                ### a_interior (fm) (entropy)
                                ### =====================================================================
                                ### predictor
                                ### =====================================================================
                                #####################################################################################################(c)
                                ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
                                array_lambda_plus_fm[j_fm+1][i_fm-1] = np.tan(array_theta_fm[j_fm+1][i_fm-1]+array_alpha_fm[j_fm+1][i_fm-1])
                                array_lambda_minus_fm[j_fm-1][i_fm] = np.tan(array_theta_fm[j_fm-1][i_fm]-array_alpha_fm[j_fm-1][i_fm])
                                ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
                                array_Q_plus_fm[j_fm+1][i_fm-1] = np.sqrt((array_M_fm[j_fm+1][i_fm-1])**2.-1.) / (array_rho_fm[j_fm+1][i_fm-1]*array_V_fm[j_fm+1][i_fm-1]**2.)
                                array_Q_minus_fm[j_fm-1][i_fm] = np.sqrt((array_M_fm[j_fm-1][i_fm])**2.-1.) / (array_rho_fm[j_fm-1][i_fm]*array_V_fm[j_fm-1][i_fm]**2.)
                                #####################################################################################################(d)
                                ### eq17dot44_eq17dot45
                                array_x_fm[j_fm][i_fm], array_y_fm[j_fm][i_fm] = func_cross_gas_dynamics(\
                                    (array_x_fm[j_fm-1][i_fm], array_y_fm[j_fm-1][i_fm]),\
                                        (array_x_fm[j_fm+1][i_fm-1], array_y_fm[j_fm+1][i_fm-1]),\
                                            array_lambda_minus_fm[j_fm-1][i_fm], \
                                                array_lambda_plus_fm[j_fm+1][i_fm-1])
                                ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
                                array_T_plus_fm[j_fm+1][i_fm-1] = array_Q_plus_fm[j_fm+1][i_fm-1] * array_p_fm[j_fm+1][i_fm-1] + array_theta_fm[j_fm+1][i_fm-1]
                                array_T_minus_fm[j_fm-1][i_fm] = array_Q_minus_fm[j_fm-1][i_fm] * array_p_fm[j_fm-1][i_fm] - array_theta_fm[j_fm-1][i_fm]
                                #####################################################################################################(e)
                                ### eq17dot49 (the modified euler predictor-corrector)
                                ### eq17dot43_eq17dot46
                                array_lambda_12_fm[j_fm-1][i_fm] = (array_y_fm[j_fm+1][i_fm-1] - array_y_fm[j_fm-1][i_fm]) / (array_x_fm[j_fm+1][i_fm-1] - array_x_fm[j_fm-1][i_fm])
                                array_lambda_o_fm[j_fm][i_fm-1] = np.tan((array_theta_fm[j_fm-1][i_fm] + array_theta_fm[j_fm+1][i_fm-1])/2.)
                                array_x_3_fm[j_fm][i_fm-1],\
                                    array_y_3_fm[j_fm][i_fm-1],\
                                        array_theta_3_fm[j_fm][i_fm-1],\
                                            array_lambda_o_fm[j_fm][i_fm-1] = func_MEPC_theta3_mode1(\
                                    array_theta_fm[j_fm-1][i_fm],\
                                        array_theta[j_fm+1][i_fm-1],\
                                            (array_x_fm[j_fm-1][i_fm], array_y_fm[j_fm-1][i_fm]),\
                                                (array_x_fm[j_fm+1][i_fm-1], array_y_fm[j_fm+1][i_fm-1]),\
                                                    (array_x_fm[j_fm][i_fm], array_y_fm[j_fm][i_fm]),\
                                                        array_lambda_12_fm[j_fm-1][i_fm],\
                                                            array_lambda_o_fm[j_fm][i_fm-1])
                                ### interpolating for the remaining flow properties gives... (p.203) 
                                array_p_3_fm[j_fm][i_fm-1] = array_p_fm[j_fm+1][i_fm-1]+(array_y_3_fm[j_fm][i_fm-1]-array_y_fm[j_fm+1][i_fm-1])/\
                                    (array_y_fm[j_fm-1][i_fm]-array_y_fm[j_fm+1][i_fm-1])*(array_p_fm[j_fm-1][i_fm]-array_p_fm[j_fm+1][i_fm-1])
                                ### 無理やりエントロピーに関しても内挿してみる
                                array_s_3_fm[j_fm][i_fm-1] = array_s_fm[j_fm+1][i_fm-1]+(array_y_3_fm[j_fm][i_fm-1]-array_y_fm[j_fm+1][i_fm-1])/\
                                    (array_y_fm[j_fm-1][i_fm]-array_y_fm[j_fm+1][i_fm-1])*(array_s_fm[j_fm-1][i_fm]-array_s_fm[j_fm+1][i_fm-1])
                                gas.SPX = array_s_3_fm[j_fm][i_fm-1], array_p_3_fm[j_fm][i_fm-1], q
                                array_rho_3_fm[j_fm][i_fm-1] = gas.density_mass
                                array_V_3_fm[j_fm][i_fm-1] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                #####################################################################################################(f)
                                array_a_fr_3_fm[j_fm][i_fm-1] = soundspeed_fr(gas)
                                ### eq.(g) & (h) for calculating p4, theta4
                                array_p_fm[j_fm][i_fm] = (array_T_plus_fm[j_fm+1][i_fm-1] + array_T_minus_fm[j_fm-1][i_fm]) / \
                                    (array_Q_plus_fm[j_fm+1][i_fm-1] + array_Q_minus_fm[j_fm-1][i_fm])
                                array_theta_fm[j_fm][i_fm] = array_T_plus_fm[j_fm+1][i_fm-1] - array_Q_plus_fm[j_fm+1][i_fm-1] * array_p_fm[j_fm][i_fm]
                                ### eq.(i) & (j) for calculating V4, rho4
                                ### 格子の交点に沿って流線が存在する　違うわ
                                gas.SPX = s_post,array_p_fm[j_fm][i_fm],q########################################################################################変更する
                                array_V_fm[j_fm][i_fm] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                array_rho_fm[j_fm][i_fm] = gas.density_mass
                                ### set predictor
                                theta_3 = array_theta_3_fm[j_fm][i_fm-1]
                                delta_c = 1.0
                                ### =====================================================================
                                ### corrector : 全て入れなおせているのだろうか？
                                ### =====================================================================
                                while delta_c >= eps_c:
                                    #####################################################################################################(g)
                                    ### along Mach line 24 (C+)
                                    array_p_plus_fm[j_fm+1][i_fm-1] = (array_p_fm[j_fm+1][i_fm-1] + array_p_fm[j_fm][i_fm]) /2.
                                    array_s_plus_fm[j_fm+1][i_fm-1] = (array_s_fm[j_fm+1][i_fm-1] + array_s_fm[j_fm][i_fm]) /2.
                                    array_theta_plus_fm[j_fm+1][i_fm-1] = (array_theta_fm[j_fm+1][i_fm-1] + array_theta_fm[j_fm][i_fm]) /2.
                                    gas.SPX = array_s_plus_fm[j_fm+1][i_fm-1], array_p_plus_fm[j+1][i-1], q
                                    array_V_plus_fm[j_fm+1][i_fm-1] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    array_rho_plus_fm[j_fm+1][i_fm-1] = gas.density_mass
                                    array_y_plus_fm[j_fm+1][i_fm-1] = (array_y_fm[j_fm+1][i_fm-1] + array_y_fm[j_fm][i_fm]) /2.
                                    array_a_fr_plus_fm[j_fm+1][i_fm-1] = soundspeed_fr(gas)
                                    array_M_plus_fm[j_fm+1][i_fm-1] = array_V_plus_fm[j_fm+1][i_fm-1] / array_a_fr_plus_fm[j_fm+1][i_fm-1]
                                    array_alpha_plus_fm[j_fm+1][i_fm-1] = np.arcsin(1./array_M_plus_fm[j_fm+1][i_fm-1])
                                    array_lambda_plus_fm[j_fm+1][i_fm-1] = np.tan(array_theta_plus_fm[j_fm+1][i_fm-1]+array_alpha_plus_fm[j_fm+1][i_fm-1])
                                    array_Q_plus_fm[j_fm+1][i_fm-1] = np.sqrt(array_M_plus_fm[j_fm+1][i_fm-1]**2.-1.) / \
                                        (array_rho_plus_fm[j_fm+1][i_fm-1]*array_V_plus_fm[j_fm+1][i_fm-1]**2.)
                                    ### along Mach line 14 (C-)
                                    array_p_minus_fm[j_fm-1][i_fm] = (array_p_fm[j_fm-1][i_fm] + array_p_fm[j_fm][i_fm]) /2.
                                    array_s_minus_fm[j_fm-1][i_fm] = (array_s_fm[j_fm-1][i_fm] + array_s_fm[j_fm][i_fm]) /2.
                                    array_theta_minus_fm[j_fm-1][i_fm] = (array_theta_fm[j_fm-1][i_fm] + array_theta_fm[j_fm][i_fm]) /2.
                                    gas.SPX = array_s_minus_fm[j_fm-1][i_fm], array_p_minus_fm[j_fm-1][i_fm], q
                                    array_V_minus_fm[j_fm-1][i_fm] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    array_rho_minus_fm[j_fm-1][i_fm] = gas.density_mass
                                    array_y_minus_fm[j_fm-1][i_fm] = (array_y_fm[j_fm-1][i_fm] + array_y_fm[j_fm][i_fm]) /2.
                                    array_a_fr_minus_fm[j_fm-1][i_fm] = soundspeed_fr(gas)
                                    array_M_minus_fm[j_fm-1][i_fm] = array_V_minus_fm[j_fm-1][i_fm] / array_a_fr_minus_fm[j_fm-1][i_fm]
                                    array_alpha_minus_fm[j_fm-1][i_fm] = np.arcsin(1./array_M_minus_fm[j_fm-1][i_fm])
                                    array_lambda_minus_fm[j_fm-1][i_fm] = np.tan(array_theta_minus_fm[j_fm-1][i_fm]-array_alpha_minus_fm[j_fm-1][i_fm])
                                    array_Q_minus_fm[j_fm-1][i_fm] = np.sqrt(array_M_minus_fm[j_fm-1][i_fm]**2.-1.) / \
                                        (array_rho_minus_fm[j_fm-1][i_fm]*array_V_minus_fm[j_fm-1][i_fm]**2.)
                                    #####################################################################################################(h)
                                    ### eq17dot44_eq17dot45
                                    array_x_fm[j_fm][i_fm], array_y_fm[j_fm][i_fm] = func_cross_gas_dynamics(\
                                        (array_x_fm[j_fm-1][i_fm], array_y_fm[j_fm-1][i_fm]),\
                                            (array_x_fm[j_fm+1][i_fm-1], array_y_fm[j_fm+1][i_fm-1]),\
                                                array_lambda_minus_fm[j_fm-1][i_fm], \
                                                    array_lambda_plus_fm[j_fm+1][i_fm-1])
                                    ### T+ & T- - eq17dot52_eq17dot53 (first step predictor
                                    array_T_plus_fm[j_fm+1][i_fm-1] = array_Q_plus_fm[j_fm+1][i_fm-1] * array_p_fm[j_fm+1][i_fm-1] + array_theta_fm[j_fm+1][i_fm-1]
                                    array_T_minus_fm[j_fm-1][i_fm] = array_Q_minus_fm[j_fm-1][i_fm] * array_p_fm[j_fm-1][i_fm] - array_theta_fm[j_fm-1][i_fm]
                                    #####################################################################################################(i)
                                    ### eq17dot49 (the modified euler predictor-corrector)
                                    ### eq17dot43_eq17dot46
                                    ### (theta3, theta4) -> corrector
                                    array_lambda_o_fm[j_fm][i_fm-1] = np.tan((array_theta_3_fm[j_fm][i_fm-1]+array_theta_fm[j_fm][i_fm])/2.)
                                    array_x_3_fm[j_fm][i_fm-1],\
                                        array_y_3_fm[j_fm][i_fm-1],\
                                            array_theta_3_fm[j_fm][i_fm-1],\
                                                array_lambda_o_fm[j_fm][i_fm-1] = func_MEPC_theta3_mode2(\
                                        array_theta_fm[j_fm-1][i_fm],\
                                            array_theta_fm[j_fm+1][i_fm-1],\
                                            array_theta_fm[j_fm][i_fm],\
                                                (array_x_fm[j_fm-1][i_fm], array_y_fm[j_fm-1][i_fm]),\
                                                    (array_x_fm[j_fm+1][i_fm-1], array_y_fm[j_fm+1][i_fm-1]),\
                                                        (array_x_fm[j_fm][i_fm], array_y_fm[j_fm][i_fm]),\
                                                            array_lambda_12_fm[j_fm-1][i_fm],\
                                                                array_lambda_o_fm[j_fm][i_fm-1])
                                    ### interpolating for the remaining flow properties gives... (p.203)
                                    array_p_3_fm[j_fm][i_fm-1] = array_p_fm[j_fm+1][i_fm-1]+(array_y_3_fm[j_fm][i_fm-1]-array_y_fm[j_fm+1][i_fm-1])/\
                                        (array_y_fm[j_fm-1][i_fm]-array_y_fm[j_fm+1][i_fm-1])*(array_p_fm[j_fm-1][i_fm]-array_p_fm[j_fm+1][i_fm-1])
                                    array_s_3_fm[j_fm][i_fm-1] = array_s_fm[j_fm+1][i_fm-1]+(array_y_3_fm[j_fm][i_fm-1]-array_y_fm[j_fm+1][i_fm-1])/\
                                        (array_y_fm[j_fm-1][i_fm]-array_y_fm[j_fm+1][i_fm-1])*(array_s_fm[j_fm-1][i_fm]-array_s_fm[j_fm+1][i_fm-1])
                                    gas.SPX = array_s_3_fm[j_fm][i_fm-1], array_p_3_fm[j_fm][i_fm-1], q
                                    array_rho_3_fm[j_fm][i_fm-1] = gas.density_mass
                                    array_V_3_fm[j_fm][i_fm-1] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    #####################################################################################################(j)
                                    ### (start) ここで計算が「predictor」と「corrector」で異なる
                                    array_p_o_fm[j_fm][i_fm-1] = (array_p_3_fm[j_fm][i_fm-1] + array_p_fm[j_fm][i_fm]) / 2.
                                    array_s_o_fm[j_fm][i_fm-1] = (array_s_3_fm[j_fm][i_fm-1] + array_s_fm[j_fm][i_fm]) / 2.
                                    gas.SPX = array_s_o_fm[j_fm][i_fm-1], array_p_o_fm[j_fm][i_fm-1], q
                                    array_rho_o_fm[j_fm][i_fm-1] = gas.density_mass
                                    array_V_o_fm[j_fm][i_fm-1] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    ### (end)
                                    array_a_fr_3_fm[j_fm][i_fm-1] = soundspeed_fr(gas)
                                    ### eq.(g) & (h) for calculating p4, theta4
                                    array_p_fm[j_fm][i_fm] = (array_T_plus_fm[j_fm+1][i_fm-1] + array_T_minus_fm[j_fm-1][i_fm]) /\
                                        (array_Q_plus_fm[j_fm+1][i_fm-1] + array_Q_minus_fm[j_fm-1][i_fm])
                                    array_theta_fm[j_fm][i_fm] = array_T_plus_fm[j_fm+1][i_fm-1] - array_Q_plus_fm[j_fm+1][i_fm-1] * array_p_fm[j_fm][i_fm]
                                    ### eq.(i) & (j) for calculating V4, rho4
                                    gas.SPX = s_post, array_p_fm[j_fm][i_fm], q#################################################################################変更する
                                    array_V_fm[j_fm][i_fm] = np.sqrt(2.0*(h_t_shock_fixed - gas.enthalpy_mass))
                                    array_rho_fm[j_fm][i_fm] = gas.density_mass
                                    ### count
                                    array_a_fr_fm[j_fm][i_fm] = soundspeed_fr(gas)
                                    array_M_fm[j_fm][i_fm] = array_V_fm[j_fm][i_fm] / array_a_fr_fm[j_fm][i_fm]
                                    array_alpha_fm[j_fm][i_fm] = np.arcsin(1./array_M_fm[j_fm][i_fm])
                                    ### delta_c
                                    theta_3_new = array_theta_3_fm[j_fm][i_fm-1]
                                    delta_c = abs((theta_3-theta_3_new)/theta_3)
                                    theta_3 = theta_3_new
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            
                            elif j_fm == j_bottom_fm:
                            
                                ### fig. 
                                ### point_1 --->>> array__fm[-2][i_fm]
                                ### point_3 --->>> array__fm[-1][i_fm]
                                
                                ### point_3 の計算開始
                                ### array_x_fm[-1][0] 更新
                                ### array_y_fm[-1][0] 更新
                                ### array_p_fm[-1][0] 更新
                                
                                ### ここの計算はあくまで未燃混合気層での計算ぞ？？？
                                ### 直接，燃焼室内の要素からインジェクター面の要素の間で計算するものではない
                                ################################################ あくまで判定する基準 & choking flow での計算（start）

                                ### M1 = 1 になるまでsecant method ???
                                M1 = 1.
                                def func_delta_P1_sonic(P1, M1=1.):
                                    mdot_over_A1 = P_ple / np.sqrt(T_ple) * np.sqrt(gamma_fr_ple/R_ple) * M1 * \
                                        (1.+(gamma_fr_ple-1.)/2.*M1**2.)**(-(gamma_fr_ple+1.)/(2.*(gamma_fr_ple-1.)))
                                    v3 = func_kainokoushiki(1./2., gamma_fr_ple/(gamma_fr_ple-1.)*P3/(A1_over_A3*mdot_over_A1), \
                                        -gamma_fr_ple/(gamma_fr_ple-1.)*P_ple*rho_ple)
                                    gas.SPX = s_ple, P1, q
                                    v1 = np.sqrt(2.0*(h_ple - gas.enthalpy_mass))
                                    a_fr_1 = soundspeed_fr(gas)
                                    delta_M1 = (M1-v1/a_fr_1)
                                    return delta_M1, v3, v1, a_fr_1,mdot_over_A1
                                P1_a = 101300*1.2
                                P1_b = 101300*1.4
                                dP1_a,v3,v1,a_fr_1,mdot_over_A1 = func_delta_P1_sonic(P1_a)
                                dP1_b,v3,v1,a_fr_1,mdot_over_A1 = func_delta_P1_sonic(P1_b)
                                while abs(dP1_b) > eps_c:
                                    P1_s = (P1_a * dP1_b - P1_b * dP1_a)/(dP1_b - dP1_a)
                                    P1_a, P1_b = P1_b, P1_s
                                    dP1_a = dP1_b
                                    dP1_b,v3,v1,a_fr_1,mdot_over_A1 = func_delta_P1_sonic(P1_b)
                                P1 = P1_b
                                Pcr = (mdot_over_A1 * (v3 - v1) - P1 + P3 / A1_over_A3) / (1./A1_over_A3 - 1.) ### P2 in an article



                                ################################################ あくまで判定する基準 & choking flow での計算（end）
                                ################################################ choking flow ではこれ以上計算する必要がない
                                if Pcr >= P1: ### subsonic inflow, iteration until P1 = P2
                                    print('subsonic')
                                    M1 = 0.8
                                    delta_P1 = 1.0
                                    while delta_P1 >= eps_c:
                                        # P1 = P_ple * (1.+(gamma_fr_ple-1.)/2.*M1) ** (-gamma_fr_ple/(gamma_fr_ple-1.)) ##################################
                                        mdot_over_A1 = P_ple / np.sqrt(T_ple) * np.sqrt(gamma_fr_ple/R_ple) * M1 * \
                                            (1.+(gamma_fr_ple-1.)/2.*M1**2.)**(-(gamma_fr_ple+1.)/(2.*(gamma_fr_ple-1.)))
                                        v3 = func_kainokoushiki(1./2., gamma_fr_ple/(gamma_fr_ple-1.)*P3/(A1_over_A3*mdot_over_A1), \
                                            -gamma_fr_ple/(gamma_fr_ple-1.)*P_ple*rho_ple)
                                        gas.SPX = s_ple, P1, q
                                        v1 = np.sqrt(2.0*(h_ple - gas.enthalpy_mass))                    
                                        a_fr_1 = soundspeed_fr(gas)
                                        M1 = v1 / a_fr_1
                                        P2 = (mdot_over_A1 * (v3 - v1) - P1 + P3 / A1_over_A3) / (1./A1_over_A3 - 1.) ### P2 in an article
                                        delta_P1 = abs((P1-P2)/P2)
                                        P1 = P2
                                    angle_bottom_new = np.arctan(((inflow_distance+v3*(array_x[-1][i]-array_x[-1][i-1])/CJ_speed) - array_y[-1][i-1]) / \
                                        (array_x[-1][i] - array_x[-1][i-1]))
                                elif Pcr < P1: ### supersonic inflow, no further calculation
                                    print('supersonic')
                                    angle_bottom_new = np.arctan(((inflow_distance+v3*(array_x[-1][i]-array_x[-1][i-1])/CJ_speed) - array_y[-1][i-1]) / \
                                        (array_x[-1][i] - array_x[-1][i-1]))
                                    judge = judge_new
                                    judge_new = 3
                                    delta_y_fm = (array_y[-1][i]-(inflow_distance+v3*(array_x[-1][i]-array_x[-1][i-1])/CJ_speed))/array_y[-1][i]
                                else:
                                    print("ERROR : inflow calculation")


                                print('subsonic /// initial_point at point_3')
                                M1 = 0.8 ### predictor of Mach_number
                                P1 = P_ple * (1.+(gamma_fr_ple-1.)/2.*M1) ** (-gamma_fr_ple/(gamma_fr_ple-1.)) * 2. ### predictor であればなんでも良いのか？？？
                                delta_P1 = 1.0
                                P3 = array_p_fm[-1][0] ### 内挿した p_13 で inflow_boudary の jump_relation 計算
                                while delta_P1 >= eps_c:
                                    mdot_over_A1 = P_ple / np.sqrt(T_ple) * np.sqrt(gamma_fr_ple/R_ple) * M1 * \
                                        (1.+(gamma_fr_ple-1.)/2.*M1**2.)**(-(gamma_fr_ple+1.)/(2.*(gamma_fr_ple-1.)))
                                    v3 = func_kainokoushiki(1./2., gamma_fr_ple/(gamma_fr_ple-1.)*P3/(A1_over_A3*mdot_over_A1), \
                                        -gamma_fr_ple/(gamma_fr_ple-1.)*P_ple*rho_ple) ### lab_fixed 
                                    gas.SPX = s_ple, P1, q
                                    v1 = np.sqrt(2.0*(h_ple - gas.enthalpy_mass)) ### lab_fixed                   
                                    a_fr_1 = soundspeed_fr(gas)
                                    M1 = v1 / a_fr_1 ### lab_fixed 
                                    P2 = (mdot_over_A1 * (v3 - v1) - P1 + P3 / A1_over_A3) / (1./A1_over_A3 - 1.) ### P2 in an article
                                    delta_P1 = abs((P1-P2)/P2)
                                    P1 = P2
                                    if M1 >= 1.:
                                        print('ERROR : 亜音速なんですけどおかしくないですか？')
                                del P3 ### 以降のエラー防止のための試み
                                
                                array_h_fm_lab_fixed[-1][0] = h_ple - v3**2./2. ### static enthalpy lab_fixed
                                gas.HPX = array_h_fm_lab_fixed[-1][0], array_p_fm[-1][0], q ### lab_fixed
                                array_s_fm[-1][0] = gas.entropy_mass ### both lab_fixed and shock_fixed, この後エントロピーの分布が生まれるってことやんね？どうせ斜め衝撃波背後の領域は entropy 分布できる
                                array_rho_fm[-1][0] = gas.density_mass ### both lab_fixed and shock_fixed
                                array_a_fr_fm[-1][0] = soundspeed_fr(gas)
                                
                                ### ここの値次第で entropy の変化を考慮すべきかどうかを判断する
                                print('s_ple =', s_ple, '/// array_s_fm[-1][0] =', array_s_fm[-1][0], '/// delta_s =', (s_ple-array_s_fm[-1][0])/array_s_fm[-1][0], '%')
                                
                                ### 
                                ### shock_fixed
                                ### theta_13 = np.arctan(v3/CJ_speed)
                                array_theta_fm[-1][0] = np.arctan(v3/CJ_speed)
                                array_V_fm[-1][0] = np.sqrt(v3**2.+CJ_speed**2.)
                                array_h_fm_shock_fixed[-1][0] = array_h_fm_lab_fixed[-1][0] + array_V_fm[-1][0]**2./2.
                                array_M_fm[-1][0] = array_V_fm[-1][0] / array_a_fr_fm[-1][0]
                                array_alpha_fm[-1][0] = np.arcsin(1./array_M_fm[-1][0])
                                
                                ###
                                ### 特性線 C- の計算
                                p_minus_23 = (array_p_fm[0][0]+array_p_fm[-1][0])/2.
                                s_minus_23 = (array_s_fm[0][0]+array_s_fm[-1][0])/2.
                                gas.SPX = s_minus_23,p_minus_23,q
                                V_minus_23 = np.sqrt(2.0*(array_h_fm_shock_fixed[-1][0]-gas.enthalpy_mass))
                                rho_minus_23 = gas.density_mass
                                a_fr_minus_23 = soundspeed_fr(gas)
                                M_minus_23 = V_minus_23 / a_fr_minus_23
                                Q_minus_23 = np.sqrt((M_minus_23)**2.-1.) / (rho_minus_23*V_minus_23**2.)
                                T_minus_23 = Q_minus_23*p_1-theta_1
                                array_p_fm[-1][0] = (T_minus_23+array_theta_fm[-1][0])/Q_minus_23

                                ### delta_p_3 更新
                                delta_p_3 = abs((p_3-array_p_fm[-1][0])/p_3)
                                p_3 = array_p_fm[-1][0]



                            
                            
                            
                        
                        
                        
                        
                        
                        
                        
                        
                        
                        
                        
                        
                        
                        
                    



















                
    print('===================================================================',i)
    ### inflow の進行距離計算
    inflow_distance += v3 * (array_x[-1][i] - array_x[-1][i-1]) / CJ_speed
    array_x_fm = np.hstack((array_x_fm, array_x[-1][i]))
    array_y_fm = np.hstack((array_y_fm, inflow_distance))
    array_theta_fm = np.hstack((array_theta_fm, array_theta[-1][i]))

### ============================================================= wall_reflection and injection judge _example 17.2_end
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### ====================================================================
### 追加の特性線の部分だけの計算
### ====================================================================

### 全てのパラメーター更新
### 追加する特性線の値
# num_ch_add = 5

def func_add_ch(array_target):
    target_sl = array_target[0][int(num_ch_up-1)]
    array_target = np.delete(array_target,0,0)
    array_target_add0 = np.linspace(target_sl,array_target[0][int(num_ch_up-1)],int(num_ch_add+1))
    array_target_add0 = np.array([np.delete(array_target_add0, -1, 0)])
    array_target_add1 = np.zeros((int(num_ch_add), int(num_ch_up-1)))
    array_target_add1 = np.hstack((array_target_add1, np.transpose(array_target_add0)))
    array_target_add1 = np.hstack((array_target_add1, np.zeros((int(num_ch_add),int(num_ch_down+num_ch_add)))))
    array_target = np.vstack((array_target_add1,array_target))
    return array_target

def func_add_ch_minus(array_target):
    array_target = np.delete(array_target,0,0)
    array_target_add0 = np.ones(int(num_ch_add+1)) * array_target[0][int(num_ch_up-1)]
    array_target_add0 = np.array([np.delete(array_target_add0, -1, 0)])
    array_target_add1 = np.zeros((int(num_ch_add), int(num_ch_up-1)))
    array_target_add1 = np.hstack((array_target_add1, np.transpose(array_target_add0)))
    array_target_add1 = np.hstack((array_target_add1, np.zeros((int(num_ch_add),int(num_ch_down+num_ch_add)))))
    array_target = np.vstack((array_target_add1,array_target))
    return array_target

### しっかりとマッハ数を linspace で等分
### その各マッハ数に応じた値を格納した方がいい

### 通常の値は M を基準に計算する？
### plus は全て存在しない
### minus は array__plus[j+1][i-1] が存在しないため内挿法等で決定するしかない
### _3, _o は存在しない

### ==========================================
### delete
### ==========================================
array_x = func_add_ch(array_x)
array_y = func_add_ch(array_y)
array_theta = func_add_ch(array_theta)
array_M = func_add_ch(array_M)
array_alpha = func_add_ch(array_alpha)
array_p = func_add_ch(array_p)
array_t = func_add_ch(array_t)
array_R = func_add_ch(array_R)
array_rho = func_add_ch(array_rho)
array_a_fr = func_add_ch(array_a_fr)
array_V = func_add_ch(array_V)
array_gamma = func_add_ch(array_gamma)

array_T_plus = func_add_ch(array_T_plus)
array_p_plus = func_add_ch(array_p_plus)
array_theta_plus = func_add_ch(array_theta_plus)
array_V_plus = func_add_ch(array_V_plus)
array_rho_plus = func_add_ch(array_rho_plus)
array_y_plus = func_add_ch(array_y_plus)
array_a_fr_plus = func_add_ch(array_a_fr_plus)
array_M_plus = func_add_ch(array_M_plus)
array_alpha_plus = func_add_ch(array_alpha_plus)
array_lambda_plus = func_add_ch(array_lambda_plus)
array_Q_plus = func_add_ch(array_Q_plus)
array_S_plus = func_add_ch(array_S_plus)

array_T_minus = func_add_ch_minus(array_T_minus)
array_p_minus = func_add_ch_minus(array_p_minus)
array_theta_minus = func_add_ch_minus(array_theta_minus)
array_V_minus = func_add_ch_minus(array_V_minus)
array_rho_minus = func_add_ch_minus(array_rho_minus)
array_y_minus = func_add_ch_minus(array_y_minus)
array_a_fr_minus = func_add_ch_minus(array_a_fr_minus)
array_M_minus = func_add_ch_minus(array_M_minus)
array_alpha_minus = func_add_ch_minus(array_alpha_minus)
array_lambda_minus = func_add_ch_minus(array_lambda_minus)
array_Q_minus = func_add_ch_minus(array_Q_minus)
array_S_minus = func_add_ch_minus(array_S_minus)

array_x_3 = func_add_ch(array_x_3)
array_y_3 = func_add_ch(array_y_3)
array_theta_3 = func_add_ch(array_theta_3)
array_p_3 = func_add_ch(array_p_3)
array_rho_3 = func_add_ch(array_rho_3)
array_V_3 = func_add_ch(array_V_3)
array_a_fr_3 = func_add_ch(array_a_fr_3)
array_gamma_3 = func_add_ch(array_gamma_3)

array_lambda_12 = func_add_ch(array_lambda_12)
array_lambda_o = func_add_ch(array_lambda_o)
array_p_o = func_add_ch(array_p_o)
array_rho_o = func_add_ch(array_rho_o)
array_V_o = func_add_ch(array_V_o)
array_A_o = func_add_ch(array_A_o)
array_R_o = func_add_ch(array_R_o)
array_T_o1 = func_add_ch(array_T_o1)
array_T_o2 = func_add_ch(array_T_o2)


### ====================================================================
### 追加の特性線の状態量を内挿した上で
### M か何かの状態量を基準に計算して上書き
### だめだ何か収束してほしい値がある場合は secant 法しか思い浮かばない
### ====================================================================
for i_add in range(num_ch_add):
    array_p[i_add][int(num_ch_up-1)], \
        array_t[i_add][int(num_ch_up-1)], \
            array_R[i_add][int(num_ch_up-1)], \
                array_rho[i_add][int(num_ch_up-1)], \
                    array_a_fr[i_add][int(num_ch_up-1)], \
                        array_V[i_add][int(num_ch_up-1)], \
                            array_gamma[i_add][int(num_ch_up-1)] = func_M2P(array_M[i_add][int(num_ch_up-1)])
print('end')




















### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### ============================================================= free boundaries condition reflection _example 17.5_start

### ====================================
### ====================================
### 仮のすべり面に沿った圧力分布の作成
### ====================================
### ====================================

array_x_sl = np.hstack((array_x_sl, array_point_dw[0]))
array_y_sl = np.hstack((array_y_sl, array_point_dw[1]))

i2=1

i3 = 0

# for i in range(int(num_ch_up),int(num_ch_up+18)):
for i in range(int(num_ch_up),int(num_ch_up+num_ch_down-2+num_ch_add)):

    print('i =',i,'upper_ref')

    j = None ### 自由境界におけるj関連のエラーを回避するため
    ### =====================================================================
    ### predictor
    ### =====================================================================
    np.savetxt('array_x.csv', array_x, delimiter=',')
    np.savetxt('array_y.csv', array_y, delimiter=',')
    np.savetxt('array_theta.csv', array_theta/2./np.pi*360., delimiter=',')
    np.savetxt('array_p.csv', array_p, delimiter=',')
    np.savetxt('array_V.csv', array_V, delimiter=',')
    np.savetxt('array_M.csv', array_M, delimiter=',')
    np.savetxt('array_lambda_plus.csv', array_lambda_plus, delimiter=',')
    np.savetxt('array_lambda_minus.csv', array_lambda_minus, delimiter=',')
    np.savetxt('array_Q_plus.csv', array_Q_plus, delimiter=',')
    np.savetxt('array_Q_minus.csv', array_Q_minus, delimiter=',')
    np.savetxt('array_T_plus.csv', array_T_plus, delimiter=',')
    np.savetxt('array_T_minus.csv', array_T_minus, delimiter=',')



    #####################################################################################################(e)
    ### 必要だけど一度削除
    ## array__3[0][i-1] = array_[0][i-1]
    array_x_3[0][i-1] = array_x[0][i-1]
    array_y_3[0][i-1] = array_y[0][i-1]
    array_theta_3[0][i-1]  = array_theta[0][i-1]
    array_p_3[0][i-1] = array_p[0][i-1]
    gas.SPX = s_post,array_p_3[0][i-1],x_post
    array_rho_3[0][i-1] = gas.density_mass
    array_V_3[0][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
    #####################################################################################################(f)
    ### delta_theta_4
    # array_p[0][i] = (array_p[0][i-1] + array_p[1][i-1])/2 ### point_2 < point_4, point_3 > point_4 であってほしい
    array_p[0][i] = (array_p[0][i-1] + 5*array_p[1][i-1])/6 ### point_2 < point_4, point_3 > point_4 であってほしい

    gas.SPX = s_post, array_p[0][i], x_post
    array_rho[0][i] = gas.density_mass #array_rho[0][i-1]
    array_V[0][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass)) #array_V[0][i-1]
    array_a_fr[0][i] = soundspeed_fr(gas)
    array_M[0][i] = array_V[0][i] / array_a_fr[0][i]
    array_alpha[0][i] = np.arcsin(1./array_M[0][i])

    #####################################################################################################(c)
    ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
    array_lambda_plus[1][i-1] = np.tan(array_theta[1][i-1]+array_alpha[1][i-1])
    ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
    array_Q_plus[1][i-1] = np.sqrt((array_M[1][i-1])**2.-1.) / (array_rho[1][i-1]*array_V[1][i-1]**2.)
    #####################################################################################################(d)
    ### eq17dot43_eq17dot44
    ### point3 : array_x&y[0][i], not array_x&y_3[][]
    array_lambda_o[0][i-1] = np.tan((array_theta_3[0][i-1]))
    array_x[0][i], array_y[0][i] = func_cross_gas_dynamics(\
        (array_x_3[0][i-1], array_y_3[0][i-1]),\
            (array_x[1][i-1], array_y[1][i-1]),\
                array_lambda_o[0][i-1], \
                    array_lambda_plus[1][i-1])
    ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
    array_T_plus[1][i-1] = -array_S_plus[1][i-1] * (array_x[0][i]-array_x[1][i-1]) + \
        array_Q_plus[1][i-1] * array_p[1][i-1] + array_theta[1][i-1]
    array_theta[0][i] = array_T_plus[1][i-1]-array_Q_plus[1][i-1] * array_p[0][i]#######################################
    # print('i =', i, '/// theta_4 =', theta_4, '/// theta_4_new =', array_theta[0][i])

    ### set predictor
    x_4 = array_x[0][i]
    delta_x_4 = 1.0
    eps_x_4 = 10e-10
    n = 0

    while delta_x_4 >= eps_x_4:
        # print(i)
        ### along Mach line 24 (C+)
        array_p_plus[1][i-1] = (array_p[1][i-1] + array_p[0][i]) /2.
        array_theta_plus[1][i-1] = (array_theta[1][i-1] + array_theta[0][i]) /2.
        gas.SPX = s_post, array_p_plus[1][i-1], x_post
        array_V_plus[1][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
        array_rho_plus[1][i-1] = gas.density_mass
        array_y_plus[1][i-1] = (array_y[1][i-1] + array_y[0][i]) /2.
        array_a_fr_plus[1][i-1] = soundspeed_fr(gas)
        array_M_plus[1][i-1] = array_V_plus[1][i-1] / array_a_fr_plus[1][i-1]
        array_alpha_plus[1][i-1] = np.arcsin(1./array_M_plus[1][i-1])
        #####################################################################################################(c)
        ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
        array_lambda_plus[1][i-1] = np.tan(array_theta_plus[1][i-1]+array_alpha_plus[1][i-1])
        ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
        array_Q_plus[1][i-1] = np.sqrt((array_M_plus[1][i-1])**2.-1.) / (array_rho_plus[1][i-1]*array_V_plus[1][i-1]**2.)
        #####################################################################################################(d)
        ### eq17dot43_eq17dot44
        ### point3 : array_x&y[0][i], not array_x&y_3[][]
        array_lambda_o[0][i-1] = np.tan((array_theta_3[0][i-1]+array_theta[0][i])/2.)
        array_x[0][i], array_y[0][i] = func_cross_gas_dynamics(\
            (array_x_3[0][i-1], array_y_3[0][i-1]),\
                (array_x[1][i-1], array_y[1][i-1]),\
                    array_lambda_o[0][i-1], \
                        array_lambda_plus[1][i-1])
        ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
        array_T_plus[1][i-1] = -array_S_plus[1][i-1] * (array_x[0][i]-array_x[1][i-1]) + \
            array_Q_plus[1][i-1] * array_p[1][i-1] + array_theta[1][i-1]
        array_theta[0][i] = array_T_plus[1][i-1]-array_Q_plus[1][i-1] * array_p[0][i]#######################################
        x_4_new = array_x[0][i]
        delta_x_4 = abs((x_4-x_4_new)/x_4)
        print('i =', i, '/// x_4 =', x_4, '/// x_4_new =', array_x[0][i], '/// delta_x_4 =', delta_x_4)
        x_4 = x_4_new

    array_x_sl = np.hstack((array_x_sl, array_x[0][i]))
    array_y_sl = np.hstack((array_y_sl, array_y[0][i]))
    print('p =', array_p[0][i], '/// T_plus =',array_T_plus[1][i-1], '/// Q_plus =',array_Q_plus[1][i-1], '/// theta =', array_theta[0][i] /2./np.pi*360.)

    # for j in range(1, 10):
    for j in range(1, int((num_ch_up+num_ch_down)-2+(num_ch_add-1))):
        ### =====================================================================
        ### predictor
        ### =====================================================================
        #####################################################################################################(c)
        ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
        array_lambda_plus[j+1][i-1] = np.tan(array_theta[j+1][i-1]+array_alpha[j+1][i-1])
        array_lambda_minus[j-1][i] = np.tan(array_theta[j-1][i]-array_alpha[j-1][i])
        ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
        array_Q_plus[j+1][i-1] = np.sqrt((array_M[j+1][i-1])**2.-1.) / (array_rho[j+1][i-1]*array_V[j+1][i-1]**2.)
        array_Q_minus[j-1][i] = np.sqrt((array_M[j-1][i])**2.-1.) / (array_rho[j-1][i]*array_V[j-1][i]**2.)
        #####################################################################################################(d)
        ### eq17dot44_eq17dot45
        array_x[j][i], array_y[j][i] = func_cross_gas_dynamics(\
            (array_x[j-1][i], array_y[j-1][i]),\
                (array_x[j+1][i-1], array_y[j+1][i-1]),\
                    array_lambda_minus[j-1][i], \
                        array_lambda_plus[j+1][i-1])
        ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
        array_T_plus[j+1][i-1] = -array_S_plus[j+1][i-1] * (array_x[j][i]-array_x[j+1][i-1]) + \
            array_Q_plus[j+1][i-1] * array_p[j+1][i-1] + array_theta[j+1][i-1]
        array_T_minus[j-1][i] = -array_S_minus[j-1][i] * (array_x[j][i]-array_x[j-1][i]) + \
            array_Q_minus[j-1][i] * array_p[j-1][i] - array_theta[j-1][i]
        #####################################################################################################(e)
        ### eq17dot49 (the modified euler predictor-corrector)
        ### eq17dot43_eq17dot46
        array_lambda_12[j-1][i] = (array_y[j+1][i-1] - array_y[j-1][i]) / (array_x[j+1][i-1] - array_x[j-1][i])
        array_lambda_o[j][i-1] = np.tan((array_theta[j-1][i] + array_theta[j+1][i-1])/2.)
        array_x_3[j][i-1],\
            array_y_3[j][i-1],\
                array_theta_3[j][i-1],\
                    array_lambda_o[j][i-1] = func_MEPC_theta3_mode1(\
            array_theta[j-1][i],\
                array_theta[j+1][i-1],\
                    (array_x[j-1][i], array_y[j-1][i]),\
                        (array_x[j+1][i-1], array_y[j+1][i-1]),\
                            (array_x[j][i], array_y[j][i]),\
                                array_lambda_12[j-1][i],\
                                    array_lambda_o[j][i-1])
        ### interpolating for the remaining flow properties gives... (p.203) 
        array_p_3[j][i-1] = array_p[j+1][i-1]+(array_y_3[j][i-1]-array_y[j+1][i-1])/(array_y[j-1][i]-array_y[j+1][i-1])*(array_p[j-1][i]-array_p[j+1][i-1])
        array_rho_3[j][i-1] = array_rho[j+1][i-1]+(array_y_3[j][i-1]-array_y[j+1][i-1])/(array_y[j-1][i]-array_y[j+1][i-1])*(array_rho[j-1][i]-array_rho[j+1][i-1])
        array_V_3[j][i-1] = array_V[j+1][i-1]+(array_y_3[j][i-1]-array_y[j+1][i-1])/(array_y[j-1][i]-array_y[j+1][i-1])*(array_V[j-1][i]-array_V[j+1][i-1])

        #####################################################################################################(f)
        gas.SPX = s_post, array_p_3[j][i-1], x_post
        array_a_fr_3[j][i-1] = soundspeed_fr(gas)
        array_R_o[j][i-1] = array_rho_3[j][i-1] * array_V_3[j][i-1]
        array_A_o[j][i-1] = array_a_fr_3[j][i-1] ** 2.
        array_T_o1[j][i-1] = array_R_o[j][i-1] * array_V_3[j][i-1] + array_p_3[j][i-1] # using state3
        array_T_o2[j][i-1] = array_p_3[j][i-1] - array_A_o[j][i-1] * array_rho_3[j][i-1] # using state3
        ### eq.(g) & (h) for calculating p4, theta4
        array_p[j][i] = (array_T_plus[j+1][i-1] + array_T_minus[j-1][i]) / (array_Q_plus[j+1][i-1] + array_Q_minus[j-1][i])
        array_theta[j][i] = array_T_plus[j+1][i-1] - array_Q_plus[j+1][i-1] * array_p[j][i]
        ### eq.(i) & (j) for calculating V4, rho4
        array_V[j][i] = (array_T_o1[j][i-1]-array_p[j][i]) / array_R_o[j][i-1]
        array_rho[j][i] = (array_p[j][i]-array_T_o2[j][i-1]) / array_A_o[j][i-1]

        ### set predictor
        theta_3 = array_theta_3[j][i-1]
        delta_c = 1.0
        eps_c = 10e-6
        n = 0
        ### =====================================================================
        ### corrector : 全て入れなおせているのだろうか？
        ### =====================================================================
        while delta_c >= eps_c:
            #####################################################################################################(g)
            ### along Mach line 24 (C+)
            array_p_plus[j+1][i-1] = (array_p[j+1][i-1] + array_p[j][i]) /2.
            array_theta_plus[j+1][i-1] = (array_theta[j+1][i-1] + array_theta[j][i]) /2.
            array_V_plus[j+1][i-1] = (array_V[j+1][i-1] + array_V[j][i]) /2.
            array_rho_plus[j+1][i-1] = (array_rho[j+1][i-1] + array_rho[j][i]) /2.
            array_y_plus[j+1][i-1] = (array_y[j+1][i-1] + array_y[j][i]) /2.
            gas.SPX = s_post, array_p_plus[j+1][i-1], x_post
            array_a_fr_plus[j+1][i-1] = soundspeed_fr(gas)
            array_M_plus[j+1][i-1] = array_V_plus[j+1][i-1] / array_a_fr_plus[j+1][i-1]
            array_alpha_plus[j+1][i-1] = np.arcsin(1./array_M_plus[j+1][i-1])
            array_lambda_plus[j+1][i-1] = np.tan(array_theta_plus[j+1][i-1]+array_alpha_plus[j+1][i-1])
            array_Q_plus[j+1][i-1] = np.sqrt(array_M_plus[j+1][i-1]**2.-1.) / (array_rho_plus[j+1][i-1]*array_V_plus[j+1][i-1]**2.)

            ### along Mach line 14 (C-)
            array_p_minus[j-1][i] = (array_p[j-1][i] + array_p[j][i]) /2.
            array_theta_minus[j-1][i] = (array_theta[j-1][i] + array_theta[j][i]) /2.
            array_V_minus[j-1][i] = (array_V[j-1][i] + array_V[j][i]) /2.
            array_rho_minus[j-1][i] = (array_rho[j-1][i] + array_rho[j][i]) /2.
            array_y_minus[j-1][i] = (array_y[j-1][i] + array_y[j][i]) /2.
            gas.SPX = s_post, array_p_minus[j-1][i], x_post
            array_a_fr_minus[j-1][i] = soundspeed_fr(gas)
            array_M_minus[j-1][i] = array_V_minus[j-1][i] / array_a_fr_minus[j-1][i]
            array_alpha_minus[j-1][i] = np.arcsin(1./array_M_minus[j-1][i])
            array_lambda_minus[j-1][i] = np.tan(array_theta_minus[j-1][i]-array_alpha_minus[j-1][i])
            array_Q_minus[j-1][i] = np.sqrt(array_M_minus[j-1][i]**2.-1.) / (array_rho_minus[j-1][i]*array_V_minus[j-1][i]**2.)

            #####################################################################################################(h)
            ### eq17dot44_eq17dot45
            array_x[j][i], array_y[j][i] = func_cross_gas_dynamics(\
                (array_x[j-1][i], array_y[j-1][i]),\
                    (array_x[j+1][i-1], array_y[j+1][i-1]),\
                        array_lambda_minus[j-1][i], \
                            array_lambda_plus[j+1][i-1])
            ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
            array_T_plus[j+1][i-1] = -array_S_plus[j+1][i-1] * (array_x[j][i]-array_x[j+1][i-1]) + \
                array_Q_plus[j+1][i-1] * array_p[j+1][i-1] + array_theta[j+1][i-1]
            array_T_minus[j-1][i] = -array_S_minus[j-1][i] * (array_x[j][i]-array_x[j-1][i]) + \
                array_Q_minus[j-1][i] * array_p[j-1][i] - array_theta[j-1][i]

            #####################################################################################################(i)
            ### eq17dot49 (the modified euler predictor-corrector)
            ### eq17dot43_eq17dot46
            ### (theta3, theta4) -> corrector
            array_lambda_o[j][i-1] = np.tan((array_theta_3[j][i-1]+array_theta[j][i])/2.)
            array_x_3[j][i-1],\
                array_y_3[j][i-1],\
                    array_theta_3[j][i-1],\
                        array_lambda_o[j][i-1] = func_MEPC_theta3_mode2(\
                array_theta[j-1][i],\
                    array_theta[j+1][i-1],\
                    array_theta[j][i],\
                        (array_x[j-1][i], array_y[j-1][i]),\
                            (array_x[j+1][i-1], array_y[j+1][i-1]),\
                                (array_x[j][i], array_y[j][i]),\
                                    array_lambda_12[j-1][i],\
                                        array_lambda_o[j][i-1])
            ### interpolating for the remaining flow properties gives... (p.203)
            array_p_3[j][i-1] = array_p[j+1][i-1]+(array_y_3[j][i-1]-array_y[j+1][i-1])/(array_y[j-1][i]-array_y[j+1][i-1])*(array_p[j-1][i]-array_p[j+1][i-1])
            array_rho_3[j][i-1] = array_rho[j+1][i-1]+(array_y_3[j][i-1]-array_y[j+1][i-1])/(array_y[j-1][i]-array_y[j+1][i-1])*(array_rho[j-1][i]-array_rho[j+1][i-1])
            array_V_3[j][i-1] = array_V[j+1][i-1]+(array_y_3[j][i-1]-array_y[j+1][i-1])/(array_y[j-1][i]-array_y[j+1][i-1])*(array_V[j-1][i]-array_V[j+1][i-1])
            #####################################################################################################(j)
            ### (start) ここで計算が「predictor」と「corrector」で異なる
            array_p_o[j][i-1] = (array_p_3[j][i-1] + array_p[j][i]) / 2.
            array_rho_o[j][i-1] = (array_rho_3[j][i-1] + array_rho[j][i]) / 2.
            array_V_o[j][i-1] = (array_V_3[j][i-1] + array_V[j][i]) / 2.
            ### (end)
            gas.SPX = s_post, array_p_o[j][i-1], x_post
            array_a_fr_3[j][i-1] = soundspeed_fr(gas)
            array_R_o[j][i-1] = array_rho_o[j][i-1] * array_V_o[j][i-1]
            array_A_o[j][i-1] = array_a_fr_3[j][i-1] ** 2.
            array_T_o1[j][i-1] = array_R_o[j][i-1] * array_V_3[j][i-1] + array_p_3[j][i-1] # using state3
            array_T_o2[j][i-1] = array_p_3[j][i-1] - array_A_o[j][i-1] * array_rho_3[j][i-1] # using state3
            ### eq.(g) & (h) for calculating p4, theta4
            array_p[j][i] = (array_T_plus[j+1][i-1] + array_T_minus[j-1][i]) / (array_Q_plus[j+1][i-1] + array_Q_minus[j-1][i])
            array_theta[j][i] = array_T_plus[j+1][i-1] - array_Q_plus[j+1][i-1] * array_p[j][i]
            ### eq.(i) & (j) for calculating V4, rho4
            array_V[j][i] = (array_T_o1[j][i-1]-array_p[j][i]) / array_R_o[j][i-1]
            array_rho[j][i] = (array_p[j][i]-array_T_o2[j][i-1]) / array_A_o[j][i-1]
            ### delta_c
            theta_3_new = array_theta_3[j][i-1]
            delta_c = abs((theta_3-theta_3_new)/theta_3)
            theta_3 = theta_3_new
            ### count
            gas.SPX = s_post, array_p[j][i], x_post
            array_a_fr[j][i] = soundspeed_fr(gas)
            array_M[j][i] = array_V[j][i] / array_a_fr[j][i]
            array_alpha[j][i] = np.arcsin(1./array_M[j][i])
            n += 1
















    ### =====================================================================
    ### predictor
    ### =====================================================================
    
    ### j のエラー検出のため
    ### j = int((num_ch_up+num_ch_down)-1
    j = None
    a = 0
    while judge != judge_new or a == 0:
        #####################################################################################################(c)
        ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
        array_lambda_minus[-2][i] = np.tan(array_theta[-2][i]-array_alpha[-2][i])
        ### Q+ & Q- - eq17dot54_eq17dot55 (first step predictor) (noting delta is removed)
        array_Q_minus[-2][i] = np.sqrt((array_M[-2][i])**2.-1.) / (array_rho[-2][i]*array_V[-2][i]**2.)
        #####################################################################################################(d)
        # ### eq17dot44_eq17dot45
        ### eq17dot44_eq17dot45--------------------------------------------------------------------------------------------------------------------変更（for wall）
        array_x[-1][i], array_y[-1][i] = func_cross_gas_dynamics(\
            (array_x[-2][i], array_y[-2][i]),\
                (array_x[-1][i-1], array_y[-1][i-1]),\
                    array_lambda_minus[-2][i], \
                        np.tan(angle_bottom))           ### ================= 条件によって変更する壁面境界条件
        ### T+ & T- - eq17dot52_eq17dot53 (first step predictor)
        array_T_minus[-2][i] = -array_S_minus[-2][i] * (array_x[-1][i]-array_x[-2][i]) + \
            array_Q_minus[-2][i] * array_p[-2][i] - array_theta[-2][i]
        #####################################################################################################(e)
        ### eq17dot49 (the modified euler predictor-corrector)
        ### eq17dot43_eq17dot46
        ### state[-1][i-1] => state3[-1][i]
        array_x_3[-1][i-1] = array_x[-1][i-1]
        array_y_3[-1][i-1] = array_y[-1][i-1]
        array_theta_3[-1][i-1] = array_theta[-1][i-1]
        array_lambda_o[-1][i-1] = array_lambda_o[-1][i-1]
        array_p_3[-1][i-1] = array_p[-1][i-1]
        array_rho_3[-1][i-1] = array_rho[-1][i-1]
        array_V_3[-1][i-1] = array_V[-1][i-1]
        #####################################################################################################(f)
        gas.SPX = s_post, array_p_3[-1][i-1], x_post
        array_a_fr_3[-1][i-1] = soundspeed_fr(gas)
        ### eq.(g) & (h) for calculating p4, theta4
        array_theta[-1][i] = angle_bottom
        array_p[-1][i] = (array_T_minus[-2][i]+array_theta[-1][i]) / array_Q_minus[-2][i] ###-------------（変更）
        ### eq.(i) & (j) for calculating V4, rho4
        gas.SPX = s_post, array_p[-1][i], x_post
        array_V[-1][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
        array_rho[-1][i] = gas.density_mass
        ### set predictor (state4)
        p_4 = array_p[-1][i]
        delta_c = 1.0
        eps_c = 10e-6

        ### =====================================================================
        ### corrector : 全て入れなおせているのだろうか？
        ### =====================================================================
        while delta_c >= eps_c:
            #####################################################################################################(g)
            ### along Mach line 24 (C+)
            ### along Mach line 14 (C-)
            array_p_minus[-2][i] = (array_p[-2][i] + array_p[-1][i]) /2.
            array_theta_minus[-2][i] = (array_theta[-2][i] + array_theta[-1][i]) /2.
            gas.SPX = s_post, array_p_minus[-2][i], x_post
            array_V_minus[-2][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
            array_rho_minus[-2][i] = gas.density_mass
            array_y_minus[-2][i] = (array_y[-2][i] + array_y[-1][i]) /2.
            array_a_fr_minus[-2][i] = soundspeed_fr(gas)
            array_M_minus[-2][i] = array_V_minus[-2][i] / array_a_fr_minus[-2][i]
            array_alpha_minus[-2][i] = np.arcsin(1./array_M_minus[-2][i])
            array_lambda_minus[-2][i] = np.tan(array_theta_minus[-2][i]-array_alpha_minus[-2][i])
            array_Q_minus[-2][i] = np.sqrt(array_M_minus[-2][i]**2.-1.) / (array_rho_minus[-2][i]*array_V_minus[-2][i]**2.)
            #####################################################################################################(h)
            ### eq17dot44_eq17dot45-------------------------------------------------------------------変更（for wall）
            array_x[-1][i], array_y[-1][i] = func_cross_gas_dynamics(\
                (array_x[-2][i], array_y[-2][i]),\
                    (array_x[-1][i-1], array_y[-1][i-1]),\
                        array_lambda_minus[-2][i], \
                            np.tan(angle_bottom))
            ### T+ & T- - eq17dot52_eq17dot53 (first step predictor
            array_T_minus[-2][i] = -array_S_minus[-2][i] * (array_x[-1][i]-array_x[-2][i]) + \
                array_Q_minus[-2][i] * array_p[-2][i] - array_theta[-2][i]
            #####################################################################################################(i)
            ### state[-1][i-1] => state3[-1][i]
            array_x_3[-1][i-1] = array_x[-1][i-1]
            array_y_3[-1][i-1] = array_y[-1][i-1]
            array_theta_3[-1][i-1] = array_theta[-1][i-1]
            array_lambda_o[-1][i-1] = array_lambda_o[-1][i-1]
            array_p_3[-1][i-1] = array_p[-1][i-1]
            array_rho_3[-1][i-1] = array_rho[-1][i-1]
            array_V_3[-1][i-1] = array_V[-1][i-1]
            #####################################################################################################(j)
            ### (start) ここで計算が「predictor」と「corrector」で異なる
            array_p_o[-1][i-1] = (array_p_3[-1][i-1] + array_p[-1][i]) / 2.
            gas.SPX = s_post, array_p_o[-1][i-1], x_post
            array_rho_o[-1][i-1] = gas.density_mass
            array_V_o[-1][i-1] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
            ### (end) 
            array_a_fr_3[-1][i-1] = soundspeed_fr(gas)
            ### eq.(g) & (h) for calculating p4, theta4
            array_theta[-1][i] = angle_bottom                    ### ================= 条件によって変更する壁面境界条件
            array_p[-1][i] = (array_T_minus[-2][i]+array_theta[-1][i]) / array_Q_minus[-2][i]
            ### eq.(i) & (j) for calculating V4, rho4
            gas.SPX = s_post, array_p[-1][i], x_post
            array_rho[-1][i] = gas.density_mass
            array_V[-1][i] = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
            array_a_fr[-1][i] = soundspeed_fr(gas)
            array_M[-1][i] = array_V[-1][i] / array_a_fr[-1][i]
            array_alpha[-1][i] = np.arcsin(1./array_M[-1][i])
            ### delta_c
            p_4_new = array_p[-1][i]
            delta_c = abs((p_4-p_4_new)/p_4)
            p_4 = p_4_new
        ### M1=1 と仮定して計算（繰り返し計算なし）
        ### M1=1 の P2 とarray_p[-1][i] を比較
        ###### (i)  choked inflow -> no further calculation
        ###### (ii) unchoked inflow -> iteration until P1 = P2
        A1_over_A3 = 0.2
        P3 = array_p[-1][i] ### point 4
        ### 現在の列の底面の要素
        if P3 >= P_ple: ### no inflow
            # angle_bottom = angle_bottom ### stay
            v3 = 0.
            judge = judge_new
            judge_new = 1
            a += 1
            delta_y_fm = 0. ### 未燃混合気相と特性線が衝突する座標が一致するまでの繰り返し計算はなし！
        else:
            ################################################ あくまで判定する基準 & choking flow での計算（start）
            def func_kainokoushiki(a, b, c):
                return (-b+np.sqrt(b**2.-4.*a*c))/2.*a
            ### M1 = 1 になるまでsecant method ???
            M1 = 1.
            def func_delta_P1_sonic(P1, M1=1.):
                mdot_over_A1 = P_ple / np.sqrt(T_ple) * np.sqrt(gamma_fr_ple/R_ple) * M1 * \
                    (1.+(gamma_fr_ple-1.)/2.*M1**2.)**(-(gamma_fr_ple+1.)/(2.*(gamma_fr_ple-1.)))
                v3 = func_kainokoushiki(1./2., gamma_fr_ple/(gamma_fr_ple-1.)*P3/(A1_over_A3*mdot_over_A1), -gamma_fr_ple/(gamma_fr_ple-1.)*P_ple*rho_ple)
                gas.SPX = s_ple, P1, q
                v1 = np.sqrt(2.0*(h_ple - gas.enthalpy_mass))
                a_fr_1 = soundspeed_fr(gas)
                delta_M1 = (M1-v1/a_fr_1)
                return delta_M1, v3, v1, a_fr_1,mdot_over_A1
            P1_a = 101300*1.2
            P1_b = 101300*1.4
            eps_P1 = 10e-6
            dP1_a,v3,v1,a_fr_1,mdot_over_A1 = func_delta_P1_sonic(P1_a)
            dP1_b,v3,v1,a_fr_1,mdot_over_A1 = func_delta_P1_sonic(P1_b)
            while abs(dP1_b) > eps_P1:
                P1_s = (P1_a * dP1_b - P1_b * dP1_a)/(dP1_b - dP1_a)
                P1_a, P1_b = P1_b, P1_s
                dP1_a = dP1_b
                dP1_b,v3,v1,a_fr_1,mdot_over_A1 = func_delta_P1_sonic(P1_b)
            P1 = P1_b
            # P1 = P_ple * (1.+(gamma_fr_ple-1.)/2.*M1) ** (-gamma_fr_ple/(gamma_fr_ple-1.))#######################################
            Pcr = (mdot_over_A1 * (v3 - v1) - P1 + P3 / A1_over_A3) / (1./A1_over_A3 - 1.) ### P2 in an article
            # print("Pcr =", Pcr, '/// P3 =', P3, '/// i =', i, 'blocked /// a =', a)
            ################################################ あくまで判定する基準 & choking flow での計算（end）
            ################################################ choking flow ではこれ以上計算する必要がない
            if Pcr >= P1: ### subsonic inflow, iteration until P1 = P2
                print('subsonic')
                M1 = 0.8
                eps_P1 = 10e-6
                delta_P1 = 1.0
                while delta_P1 >= eps_P1:
                    # P1 = P_ple * (1.+(gamma_fr_ple-1.)/2.*M1) ** (-gamma_fr_ple/(gamma_fr_ple-1.)) ##################################
                    mdot_over_A1 = P_ple / np.sqrt(T_ple) * np.sqrt(gamma_fr_ple/R_ple) * M1 * \
                        (1.+(gamma_fr_ple-1.)/2.*M1**2.)**(-(gamma_fr_ple+1.)/(2.*(gamma_fr_ple-1.)))
                    v3 = func_kainokoushiki(1./2., gamma_fr_ple/(gamma_fr_ple-1.)*P3/(A1_over_A3*mdot_over_A1), -gamma_fr_ple/(gamma_fr_ple-1.)*P_ple*rho_ple)
                    gas.SPX = s_ple, P1, q
                    v1 = np.sqrt(2.0*(h_ple - gas.enthalpy_mass))                    
                    a_fr_1 = soundspeed_fr(gas)
                    M1 = v1 / a_fr_1
                    P2 = (mdot_over_A1 * (v3 - v1) - P1 + P3 / A1_over_A3) / (1./A1_over_A3 - 1.) ### P2 in an article
                    delta_P1 = abs((P1-P2)/P2)
                    P1 = P2
                angle_bottom_new = np.arctan(((inflow_distance+v3*(array_x[-1][i]-array_x[-1][i-1])/CJ_speed) - array_y[-1][i-1]) / \
                    (array_x[-1][i] - array_x[-1][i-1]))
                judge = judge_new
                judge_new = 2
                delta_y_fm = (array_y[-1][i]-(inflow_distance+v3*(array_x[-1][i]-array_x[-1][i-1])/CJ_speed))/array_y[-1][i]
                # print(delta_y_fm,'subsonic')
                if abs(delta_y_fm) <= 10e-6:
                    a += 1
                else:
                    angle_bottom = (angle_bottom+angle_bottom_new)/2.
                ### 未燃混合気相と特性線が衝突する座標が一致するまでの繰り返し計算はなし！
                # print("P1 & P2 =", P1, '/// P3 =', P3, '/// i =', i, 'subsonic /// a =', a)
            elif Pcr < P1: ### supersonic inflow, no further calculation
                print('supersonic')
                angle_bottom_new = np.arctan(((inflow_distance+v3*(array_x[-1][i]-array_x[-1][i-1])/CJ_speed) - array_y[-1][i-1]) / \
                    (array_x[-1][i] - array_x[-1][i-1]))
                judge = judge_new
                judge_new = 3
                delta_y_fm = (array_y[-1][i]-(inflow_distance+v3*(array_x[-1][i]-array_x[-1][i-1])/CJ_speed))/array_y[-1][i]
                # print(delta_y_fm,'supersonic')
                if abs(delta_y_fm) <= 10e-6:
                    a += 1
                else:
                    angle_bottom = (angle_bottom+angle_bottom_new)/2.
                ### 未燃混合気相と特性線が衝突する座標が一致するまでの繰り返し計算はなし！
                # print("P1 & P2 =", P1, '/// P3 =', P3, '/// i =', i, 'supersonic /// a =', a)
            else:
                print("ERROR : inflow calculation")
    print('===================================================================',i)
    ### inflow の進行距離計算
    inflow_distance += v3 * (array_x[-1][i] - array_x[-1][i-1]) / CJ_speed
    array_x_fm = np.hstack((array_x_fm, array_x[-1][i]))
    array_y_fm = np.hstack((array_y_fm, inflow_distance))
    array_theta_fm = np.hstack((array_theta_fm, array_theta[-1][i]))
    i2 += 1





### ============================================================= 滑り面＆底面の間の特性線_start
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### ============================================================= 滑り面＆底面の間の特性線_end
from scipy import interpolate
### 滑り面：
# func_fm_x_y = interpolate.PchipInterpolator(array_x_fm, array_y_fm)
# func_fm_x_theta = interpolate.PchipInterpolator(array_x_fm, array_theta_fm)

### 底面：


np.savetxt('array_x_fm.csv', array_x_fm)
np.savetxt('array_y_fm.csv', array_y_fm)
np.savetxt('array_theta_fm.csv', array_theta_fm)


















### ============================================================= free boundaries condition reflection _example 17.5_start
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### =============================================================
### ============================================================= region between oblique shock and selip wall _example 17.5_start


### 二周目の滑り面の作成
### 仮で x = 0.45 にデトネーション波



np.savetxt('array_x.csv', array_x, delimiter=',')
np.savetxt('array_y.csv', array_y, delimiter=',')
np.savetxt('array_theta.csv', array_theta/2./np.pi*360., delimiter=',')
np.savetxt('array_p.csv', array_p, delimiter=',')
np.savetxt('array_V.csv', array_V, delimiter=',')
np.savetxt('array_M.csv', array_M, delimiter=',')
np.savetxt('array_lambda_plus.csv', array_lambda_plus, delimiter=',')
np.savetxt('array_lambda_minus.csv', array_lambda_minus, delimiter=',')
np.savetxt('array_Q_plus.csv', array_Q_plus, delimiter=',')
np.savetxt('array_T_plus.csv', array_T_plus, delimiter=',')










# print("/// array_x ///", array_x.shape)
# print(array_x)

# print("/// array_y ///", array_y.shape)
# print(array_y)

# print("/// array_theta ///", array_theta.shape)
# print(array_theta/2./np.pi*360.)

# print("/// array_M ///", array_M.shape)
# print(array_M)

# print("/// array_alpha ///", array_alpha.shape)
# print(array_alpha/2./np.pi*360.)

# print("/// array_V ///", array_V.shape)
# print(np.round(array_V,0))

# print("/// array_lambda_plus ///", array_lambda_plus.shape)
# print(array_lambda_plus)

# print("/// array_lambda_minus ///", array_lambda_minus.shape)
# print(array_lambda_minus)

# print("/// array_lambda_o ///", array_lambda_o.shape)
# print(array_lambda_o)

graph0.func_scatter_add(array_x,array_y)
graph0.func_plot_add(array_x_fm,array_y_fm, color='r')
graph0.func_plot_add(array_x_sl,array_y_sl, color='r')

graph0.func_show()














