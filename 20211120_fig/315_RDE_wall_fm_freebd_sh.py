...
Ca = (angle_fm - angle_fm*(1+ang_ratio)) / (a1[0]-a1[-1])
Cb = angle_fm - Ca * a1[0]
a1 = Ca * a1 + Cb
a1 = np.delete(a1,-1,0)
...
for i in range(int(num_ch_up),int(2*num_ch_up-2)):###20211029_sawada_とりあえず謎の妥協-2
    ...
    while delta_theta_4 >= eps_theta_4:
        ...
        array_p_3[0][i-1] = array_p[0][i-1] #* (1.+ 1.*i)
    ...
    print(array_p[0][i], '///', array_theta[0][i] /2./np.pi*360.)
    ...
    # for j in range(1, 10):
    for j in range(1, int((num_ch_up+num_ch_down)-2-i2)):
