...
def func_P2M(p):
    ...
    V = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
    T = gas.T
...
P_sl,t,R,rho,a_fr,V,gamma = func_M2P(M_sl)
del t; del R; del rho; del a_fr; del V; del gamma
array_p_up = np.linspace(P_post,P_sl,num_ch_up)
        ...
        array_t_up[i0], \
            array_R_up[i0], \
                array_rho_up[i0], \
                    array_a_fr_up[i0], \
                        array_V_up[i0], \
                            array_gamma_up[i0] = func_P2M(array_p_up[i0])
...
P_bottom,t,R,rho,a_fr,V,gamma = func_M2P(M_bottom)
del t; del R; del rho; del a_fr; del V; del gamma
array_p_down = np.linspace(P_post,P_bottom,num_ch_down)
        ...
        array_t_down[i0], \
            array_R_down[i0], \
                array_rho_down[i0], \
                    array_a_fr_down[i0], \
                        array_V_down[i0], \
                            array_gamma_down[i0] = func_P2M(array_p_down[i0])
...
for i in range(int(num_ch_up),int(2*num_ch_up-2)):###20211029_sawada_とりあえず謎の妥協-2
    ...
    # for j in range(1, 10):
    for j in range(1, int((num_ch_up+num_ch_down)-2)):
        ### =====================================================================
        ### predictor
        ### =====================================================================
        #####################################################################################################(c)
