...
def func_delta_M(neu_target, M, gamma=gamma_eq_post): ### あくまでデトネーション波背後の計算のためgamma_eq_postで固定
...
### 未燃混合気層
### 既燃・未燃ガス層に挟まれた滑り面から特性線が発生する仮定
### 

num_ch_fm = 4

#============================================================================
### こいつらは未燃混合気層の初期の特性線を与えるときに作成する
# array_theta_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_neu_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_M_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_alpha_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_p_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_t_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_R_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_rho_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_a_fr_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_V_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
# array_gamma_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))

### C+ 上のパラメーター？？？
array_T_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_p_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_theta_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_V_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_rho_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_y_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_a_fr_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_M_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_alpha_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_lambda_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_Q_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_S_plus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))

### C- 上のパラメーター？？？
array_T_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_p_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_theta_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_V_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_rho_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_y_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_a_fr_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_M_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_alpha_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_lambda_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_Q_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_S_minus_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))

### point3
array_x_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_y_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_theta_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_p_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_rho_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_V_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_a_fr_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_gamma_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))

### point_o
array_lambda_12_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_lambda_o_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_p_o_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_rho_o_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_V_o_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_a_fr_3_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_R_o_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_A_o_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_T_o1_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
array_T_o2_fm = np.zeros((int(num_ch_fm), int(num_ch_fm+1)))
...
for i in range(1,int(num_ch_up)):### 20211022_sawada : 次の列の計算をしていないためにエラーが起きている
    ...
    while judge != judge_new or a == 0:
        ...
        ### 現在の列の底面の要素
        if array_p[-1][i] >= P_ple: ### no inflow
        ...
        ### これ以降の P3 は未燃混合気層の行列の底面の値
        ### 底部圧力との比較対象が未燃混合気層のものになるから

        ### 初めて inflow が発生する「1点目」
        elif array_p[-1][i] < P_ple and judge_init == 0:
            judge_init += 1  ### 何点目の操作であるのかを判断するためのパラメータ
            ### リーマン不変量を通して初期特性線を描画
            ### 最後の要素の圧力差が
            theta_fm_init = array_theta[-1][i] ### 滑り面上側と未燃混合気層側の流線の傾きが一致
            p_fm_init = array_p[-1][i] ### 滑り面上側と未燃混合気層側の圧力が一致
            gas.SPX = s_pre, p_fm_init, q ### plenum条件でのガスを用いる（未燃混合気層）
            rho_fm_init = gas.density_mass
            V_fm_init = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
            a_fr_fm_init = soundspeed_fr(gas)
            gamma_fm_init = a_fr_fm_init**2.*rho_fm_init/p_fm_init
            M_fm_init = V_fm_init / a_fr_fm_init
            alpha_fm_init = np.arcsin(1./M_fm_init)
            neu_fm_init = func_M2neu(M_fm_init, gamma_fm_init)

            ### fm 層追加
            array_x_fm = np.hstack(array_x_fm,array_x[-1][i])
            array_y_fm = np.hstack(array_y_fm,array_y[-1][i])
        ...
        ### inflow が発生する「blocked inflow」&「1点目」まではこれまでの計算と同じ
        ...
        ### もうむりやり計算いたします
        ### 未燃混合気層の初期値を与えるための「2点目」
        elif array_p[-1][i] < P_ple and judge_init == 1:
            judge_init += 1 ### judge_init >= 2 にすることで以降反射しか行わないようにする

            ### =====================================================================
            ### predictor
            ### =====================================================================
            
            angle_bottom = theta_fm_init * 1.1 ### predictor of angle_bottom

            ### j のエラー検出のため
            ### j = int((num_ch_up+num_ch_down)-1
            j = None
            a = 0
            delta_angle_bottom=1.
            ### 滑り面上側の圧力とリーマン不変量最後の要素の圧力が一致するまで繰り返し計算
            while delta_angle_bottom >= 10e-6:
                ### 一旦，ある角度での滑り面上側の圧力を収束させて計算する必要がある
                while judge != judge_new or a == 0:
                    #####################################################################################################(c)
                    ### lambda_plus & lambda_minus - eq17dot47_eq17dot48 (first step predictor)
                    array_lambda_minus[-2][i] = np.tan(array_theta[-2][i]-array_alpha[-2][i])
                ...
                ### C- の特性線を通過すると仮定
                ### ごめん，とりあえず単体で計算させる
                ### angle_bottom で繰り返し計算したいから angle_bottom を出力にしたい
                ### リーマン不変量で変化した後の圧力が一致するときの angle_bottom を出力したいところ
                gas.SPX = s_ple, array_p[-1][i], q ### 圧力がarray_p[-1][i]になる未燃でのneu
                V_last_fm = np.sqrt(2.0*(h_post_U_post - gas.enthalpy_mass))
                rho_last_fm = gas.density_mass
                a_fr_last_fm = soundspeed_fr(gas)
                M_last_fm = V_last_fm / a_fr_last_fm
                gamma_last_fm = a_fr_last_fm**2.*rho_last_fm/array_p[-1][i]
                neu_last_fm = func_M2neu(M_last_fm, gamma_last_fm)
                
                theta_last_fm = theta_fm_init + neu_fm_init - neu_last_fm
                
                ### delta_angle_bottom
                delta_angle_bottom = abs(angle_bottom-theta_last_fm)/angle_bottom
                ### angle_bottom 更新
                angle_bottom = theta_last_fm

            ### すでに角度は決定しているはず
            ### C- の特性線を通過すると仮定
            ### ごめん，とりあえず作り直しの状態になるけど進める
            array_theta_fm_up = np.linspace(theta_fm_init, angle_bottom, int(num_ch_fm+1))
            array_neu_fm_up = theta_fm_init + neu_fm_init - array_theta_fm_up ### neu + theta = const.
            array_M_fm_up = np.zeros(int(num_ch_fm+1))
            array_alpha_fm_up = np.zeros(int(num_ch_fm+1))
            array_p_fm_up = np.zeros(int(num_ch_fm+1))
            array_t_fm_up = np.zeros(int(num_ch_fm+1))
            array_R_fm_up = np.zeros(int(num_ch_fm+1))
            array_rho_fm_up = np.zeros(int(num_ch_fm+1))
            array_a_fr_fm_up = np.zeros(int(num_ch_fm+1))
            array_V_fm_up = np.zeros(int(num_ch_fm+1))
            array_gamma_fm_up = np.zeros(int(num_ch_fm+1))

            for i_fm in range(num_ch_fm):
                array_M_fm_up[i_fm] = func_neu2M(array_neu_fm_up[i_fm])
                array_alpha_fm_up[i_fm] = np.arcsin(1./array_M_up[i_fm])
                array_p_fm_up[i_fm], \
                    array_t_fm_up[i_fm], \
                        array_R_fm_up[i_fm], \
                            array_rho_fm_up[i_fm], \
                                array_a_fr_fm_up[i_fm], \
                                    array_V_fm_up[i_fm], \
                                        array_gamma_fm_up[i_fm] = func_M2P(array_M_fm_up[i_fm])
            
            ### flipud は繰り返し計算が完了してから
            array_theta_fm = np.flipud(np.diag(array_theta_fm))
            array_neu_fm = np.flipud(np.diag(array_neu_fm))
            array_M_fm = np.flipud(np.diag(array_M_fm))
            array_alpha_fm = np.flipud(np.diag(array_alpha_fm))
            array_p_fm = np.flipud(np.diag(array_p_fm))
            array_t_fm = np.flipud(np.diag(array_t_fm))
            array_R_fm = np.flipud(np.diag(array_R_fm))
            array_rho_fm = np.flipud(np.diag(array_rho_fm))
            array_a_fr_fm = np.flipud(np.diag(array_a_fr_fm))
            array_V_fm = np.flipud(np.diag(array_V_fm))
            array_gamma_fm = np.flipud(np.diag(array_gamma_fm))
        ...
        ### 3点目が必要
        ### 横一列ずつ計算していく
        elif array_p[-1][i] < P_ple and judge_init >= 2:
            ### まずは3点目の計算
